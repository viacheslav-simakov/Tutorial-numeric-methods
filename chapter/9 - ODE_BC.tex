...
$w(x_{i+1/2})$:
\begin{gather}\label{eq:ODE+BC:w}
w_{i+1/2}=-\sigma_{i}\cdot(u_{i+1}-u_{i})
\end{gather}
где 
...
$\sigma_i$ -- параметр, имеющий смысл тепловой проводимости
среды на участке $x_i\leqslant x \leqslant x_{i+1}$:
\begin{equation*}
\sigma_i=\dfrac{k(x_i)}{x_{i+1}-x_i}
\end{equation*} 
...
\begin{gather}
w_{i-1/2}=-\sigma_{i-1}\cdot(u_{i}-u_{i-1})
\end{gather}
...
\mathring{\sigma}_i\cdot u_i,
\end{gather*}
где $\mathring{\sigma}_i$ -- параметр имеющий смысл
проводимости контактной области между средой и окружением:
\begin{gather*}
\mathring{\sigma}_i=\intfcenter{r(x)}.
\end{gather*}

Для определения контактной проводимости среды
$\mathring{\sigma}_i$  используем кусочную аппроксимацию
(рисунок \ref{fig:ODE+BC:approx integral}), 
...
\begin{split}
\mathring{\sigma}_i&=\intflefthalf{r(x)}+\intfrighthalf{r(x)}=\\[1ex]
&=r(x_i)\cdot(x_i-x_{i-1/2})+r(x_{i+1})\cdot(x_{i+1/2}-x_i)=\\[1em]
...
Суммарное количество тепла $\overline{q}_i$,
...
% <g>
\overline{q}_i=\intfcenter{g(x)}\approx
\dfrac{g_{i}h_i+g_{i+1}h_{i+1}}{2}
\end{gather}
где $q_i=q(x_i)$ -- плотность распределенных источников тепла 
в узлах сетки $\{x_i\}$.
