...
% u(x)
\begin{scope}[shading=ball,ball color=darkblue,color=darkblue,on background layer]
...
% w(i+1/2)
\begin{scope}[shading=ball,ball color=darkred,color=darkred,on background layer]]
...
для расчёта неизвестных функций $w_{i+1/2}$ и $u_i$, 
используя линейную связь между ними:
\begin{equation*}
\left\{\begin{matrix}[rcl]
...
u_{i+1}-u_i&=&(\alpha^{-1}_{i+1}-1)\cdot u_i - \alpha^{-1}_{i+1}\cdot\beta_{i+1}\\
 u_i&=&(\alpha^{-1}_{i+1}-1)^{-1}\cdot
 \left(\alpha^{-1}_{i+1}\cdot\beta_{i+1}-\sigma^{-1}(x_i)\cdot w_{i+1/2}\right)\\
\end{matrix}\right.
\end{equation}
...
\begin{equation*}
w_{i+1/2} - w_{i-1/2} 
+ \mathring{\alpha}_{i+1}\cdot w_{i+1/2} - \mathring{\beta}_{i+1} 
= \varepsilon(x_i),
\end{equation*}
где $\mathring{\beta}_i$ и $\mathring{\alpha}_i$ --
потоковые коэффициенты прогонки:
\begin{equation}
\mathring{\alpha}_i=\dfrac{1}{(1-\alpha_i)}\cdot
\dfrac{\mathring{\sigma}(x_i)}{\sigma(x_{i-1})},
\quad
\mathring{\beta}_i=\beta_i\cdot
\dfrac{\mathring{\sigma}(x_i)}{1-\alpha_i}
%+\dfrac{\mathring{\sigma}(x_i)}{1-\alpha_i}\cdot\left(\beta_i-\rho(x_{i-1})\cdot w_{i-1/2}\right)
\end{equation}

Из последнего соотношения можно найти рекуррентные соотношение 
для определения неизвестных:
\begin{equation}
w_{i-1/2}=\bar{\alpha}_{i+1}\cdot w_{i+1/2}
+ \bar{\beta}_{i+1}\\
\end{equation}
