...
>={Straight Barb[angle=45:2mm 1]},shorten >=1pt,
every join/.style=->,
startcycle/.style={
	draw,
	shape=trapezium,
...
% alpha[i+1], beta[i+1]
\node[signalnode,on chain,join,continue chain=going right] (SC)
{$i=1,2,\ldots,n$};
\node[on chain,rectnode,join,continue chain=going below] (ABI)
{$\alpha_{i+1}=\dfrac{b_i}{c_i-a_i\cdot\alpha_{i}}$,\quad
$\beta_{i+1}=\dfrac{a_i\cdot\beta_i+y_i}{c_i-a_i\cdot\alpha_{i}}$};
...
{$u_n=\dfrac{a_n\cdot\beta_n+y_n}{c_n-a_n\cdot\alpha_n}$};
% u[i]
\node[signalnode,on chain,join,continue chain=going below,fill=gray!10] (SC)
{$i=(n-1),\ldots,1,0$};
\end{tikzpicture}
