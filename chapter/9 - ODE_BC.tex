...
\tikzset{
	...
	startcyclenode/.style={
		draw,text width=4cm,align=center,
...
{$\beta_{1}=\dfrac{y_0}{c_0}$};
% начало цикла
\begin{scope}[every node/.style={fill=gray!10}]
\node[startcyclenode,on chain,join,text width=4cm]
{расчёт $\alpha$, $\beta$\\$i=1,2,\ldots,(n-1)$};
% alpha[i] и beta[i]
\node[rectnode,on chain,join]
...
% конец цикла
\node[endcyclenode,on chain,join]{расчёт $\alpha$, $\beta$};
\end{scope}
% соединитель
\node[circlenode,on chain,join,fill=red!10]{2};
\end{tikzpicture}
% *******************************************
...
% соединитель
\node[circlenode,on chain,join]{2};
% u[n]
\node[rectnode,on chain,join]
{$u_n=\dfrac{a_n\cdot\beta_n+y_n}{c_n-a_n\cdot\alpha_n}$};
% u[n-1]
...
% w[n-1]
\node[rectnode,on chain,join]
{$w_{n-1/2}=-\sigma_{n-1}\cdot(u_n - u_{n-1})$};
% w[n]
\node[rectnode,on chain,join]
{$w_n=w_{i-1/2}-\mathring{\sigma}_n\cdot u_n+\varepsilon_n$};
% начало цикла
\begin{scope}[every node/.style={fill=gray!10}]
\node[startcyclenode,on chain,join,text width=4cm]
{расчёт $u$, $w$\\$i=(n-1),\ldots,2,1$};
% u[i-1]
\node[rectnode,on chain,join]
{$u_{i-1}=\alpha_i\cdot u_i - \beta_i$};
% w[i-1/2]
\node[rectnode,on chain,join]
{$w_{i-1/2}=w_{i+1/2}+\mathring{\sigma}_i\cdot u_i-\varepsilon_i$};
% конец цикла
\node[endcyclenode,on chain,join]{расчёт $u$, $w$};
\end{scope}
% соединитель
\node[circlenode,on chain,join,fill=red!10]{3};
\end{tikzpicture}
\caption{Блок-схема алгоритма вычисления
неизвестной величины $w(x)$}
\end{figure}
