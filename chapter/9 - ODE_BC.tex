...
% *** Алгоритм равномерной сетки x[i]***
\begin{figure}[H]\centering
\begin{tikzpicture}[
font=\small,
...
равномерной расчетной сетки $\{x_i\}$ 
на отрезке $[0,\ell]$ с заданным количеством интервалов $n$}
...
\begin{equation*}
\sigma(x_i)=\dfrac{2k(x_i)\cdot k(x_{i+1})}{k(x_i)+k(x_{i+1})}
\cdot\dfrac{1}{x_{i+1}-x_i}
\end{equation*} 
...
$\{\alpha_i\}=(\alpha_1,\alpha_2,\ldots,\alpha_n)$ и
$\{\beta_i\}=(\beta_1,\beta_2,\ldots,\beta_n)$}
\label{fig:ODE+BC:scheme shuttle:alpha+beta}
\end{figure}
...
$\{u_i\}=(u_0,u_1,\ldots,u_n)$ в узлах расчетной сетки
...
% *** Алгоритм вычислений неизвестных u(x) и w(x) ***
\begin{figure}[H]\centering
\begin{tikzpicture}[
font=\small,
...
\end{tikzpicture}
\caption{Блок-схема алгоритма вычислений\linebreak
неизвестных краевой задачи $u$ и $w$}
\end{figure}
