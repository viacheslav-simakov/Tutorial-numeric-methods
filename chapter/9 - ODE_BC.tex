...
&=\dfrac{r(x_i)\cdot(x_i-x_{i-1})+r(x_{i+1})\cdot(x_{i+1}-x_i)}{2}.
\end{split}
\end{equation}
...
для расчёта неизвестной функции $u_i$
и её потока $w_{i-1/2}$, используя линейную связь между ними:
\begin{equation*}
\left\{\begin{matrix}[rcl]
...
\left\{\begin{matrix}[rcl]
u_i-u_{i-1}&=&(1-\alpha_i)\cdot u_i - \beta_i\\
 u_i&=&(1-\alpha_i)^{-1}\cdot\left(\beta_i-\rho(x_{i-1})\cdot w_{i-1/2}\right)\\
\end{matrix}\right.,
\end{equation}
где $\rho(x_{i-1})=\sigma^{-1}(x_{i-1})$ -- коэффициент
численно равный тепловому сопротивлению среды
на участке $x_{i-1}\leqslant x < x_i$.

Баланс тепловой энергии \eqref{eq:ODE+BC:mesh element}
с учетом линейной связи \eqref{eq:ODE+BC:shuttle flow}
неизвестной функции и её потока:
\begin{equation}
w_{i+1/2} - w_{i-1/2} 
%+\dfrac{\mathring{\sigma}(x_i)}{1-\alpha_i}\cdot\left(\beta_i-\rho(x_{i-1})\cdot w_{i-1/2}\right)
+\mathring{\beta}_i - \mathring{\alpha}_i\cdot w_{i-1/2}
= \varepsilon(x_i),
\end{equation}
где $\mathring{\beta}_i$ и $\mathring{\alpha}_i$ --
потоковые коэффициенты прогонки:
\begin{equation}
\mathring{\beta}_i=\beta_i\cdot\dfrac{\mathring{\sigma}(x_i)}{1-\alpha_i}
%+\dfrac{\mathring{\sigma}(x_i)}{1-\alpha_i}\cdot\left(\beta_i-\rho(x_{i-1})\cdot w_{i-1/2}\right)
\end{equation}



Рекуррентные соотношение для нахождения потока 
можно записать в виде:
\begin{equation}
w_{i-1/2}=w_{i+1/2}+ \mathring{\sigma}(x_i)\cdot\dfrac{\beta_i}{1-\alpha_i} 
%= \left(
%1+\dfrac{\mathring{\sigma}(x_i)\cdot\rho(x_{i-1})}{1-\alpha_i}
%\right)\cdot w_{i-1/2}
%- \mathring{\sigma}(x_i)\cdot\dfrac{\beta_i}{1-\alpha_i}
%+\dfrac{\mathring{\sigma}(x_i)}{1-\alpha_i}\cdot\left(\beta_i-\rho(x_{i-1})\cdot w_{i-1/2}\right)
- \varepsilon(x_i)\\
\end{equation}
