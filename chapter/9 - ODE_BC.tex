...
\begin{equation*}
\left\{\begin{matrix}[rcl]
-\sigma(x_0)\cdot(u_1-u_0)-(s_0-p_0\cdot u_0)
...
+ \mathring{\sigma}(x_i)\cdot u_i
&=&\varepsilon(x_i)\\
(p_{\ell}\cdot u_n-s_{\ell})+\sigma(x_{n-1})\cdot(u_n-u_{n-1})+
...
&
b_0=\sigma(x_0),&
c_0=b_0+\mathring{\sigma}(x_0)+p_0,&
...
a_n=\sigma(x_{n-1}),&
&
c_n=a_n+\mathring{\sigma}(x_n)+p_{\ell},&
...
w_{1/2}=w_0-\mathring{\sigma}(x_0)\cdot u_0+\varepsilon(x_0)\\
w_{i+1/2}=w_{i-1/2} - \mathring{\sigma}(x_i)\cdot u_i + \varepsilon(x_i),&i=1,2,\ldots,(n-1)\\
w_n=w_{n-1/2}-\mathring{\sigma}(x_n)\cdot u_n + \varepsilon(x_n)\\
\end{cases}
\end{gather}
...
%\node[cyclenode,on chain,join,continue chain=going below] (SC)
\node[cyclestart,on chain,join,continue chain=going below] (SC)
{$i=(n-1),\ldots,1,0$};
\node[rectnode,on chain,join] (UI)
{$u_i=\alpha_{i+1}\cdot u_{i+1} + \beta_{i+1}$};
\node[cycleend,on chain,join,continue chain=going below]
{конец цикла};
% стрелка
