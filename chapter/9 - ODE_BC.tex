...
\item
Находится количество узлов $p_j$ 
равномерной сетки $\{\mathring{x}_i\}$,
...
\begin{equation*}
\{\hat{x}_{i}\}_j=\xi_j+\dfrac{i}{p_j}\cdot(\xi_{j+1}-\xi_j),
\quad i=0,1,\ldots,p_j-1,
\end{equation*}
...
Применяя левостороннюю формулу прямоугольников 
...
w_{i+1/2}=-\sigma(x_{i})\cdot(u_{i+1}-u_{i})
\end{gather}
где 
...
$\sigma$ -- параметр, имеющий смысл тепловой проводимости
среды на участке $x_i\leqslant x < x_{i+1}$:
\begin{equation*}
\sigma(x_i)=\dfrac{k(x_i)}{x_{i+1}-x_i}
\end{equation*} 
...
\begin{gather}
w_{i-1/2}=-\sigma(x_{i-1})\cdot(u_{i}-u_{i-1})
\end{gather}
...
\mathring{\sigma}(x_i)\cdot u_i,
\end{gather*}
где $\mathring{\sigma}$ -- параметр имеющий смысл
проводимости контактной области между средой и окружением:
\begin{gather*}
\mathring{\sigma}(x_i)=\intfcenter{r(x)}.
\end{gather*}

Для определения контактной проводимости среды
$\mathring{\sigma}$  используем кусочную аппроксимацию
(рисунок \ref{fig:ODE+BC:approx integral}), 
...
\begin{split}
\mathring{\sigma}(x_i)&=\intflefthalf{r(x)}+\intfrighthalf{r(x)}\approx\\[1ex]
&\approx r(x_i)\cdot(x_i-x_{i-1/2})+r(x_{i+1})\cdot(x_{i+1/2}-x_i)=\\[1em]
&=r(x_i)\cdot\dfrac{x_i-x_{i-1}}{2}+r(x_{i+1})\cdot\dfrac{x_{i+1}-x_i}{2}.
\end{split}
\end{equation}
...
%&\approx\dfrac{q(x_i)\cdot(x_i-x_{i-1})+q(x_{i+1})\cdot(x_{i+1}-x_i)}{2}.
&\approx q(x_i)\cdot\dfrac{x_i-x_{i-1}}{2}+q(x_{i+1})\cdot\dfrac{x_{i+1}-x_i}{2}.
\end{split}
\end{gather}

Таким образом, баланс тепловой энергии в контрольной области
...
w_{i+1/2}=-\sigma(x_{i})\cdot(u_{i+1}-u_i)\\
w_{i-1/2}=-\sigma(x_{i-1})\cdot(u_i-u_{i-1})
\end{cases}
\end{gather} 
...
w_{1/2}-w_0+\mathring{\sigma}(x_0)\cdot u_0=\varepsilon(x_0),
\end{gather}
где
$w_{1/2}$ -- тепловой поток в первом промежуточном 
узле $x_{1/2}$:
\begin{equation}\label{eq:ODE+BC:w(1/2)} 
w_{1/2} = -\sigma(x_0)\cdot(u_1-u_0);
\end{equation}
$w_0$ -- тепловой поток на левой границе области
\eqref{eq:HT+BC:1D}:
\begin{equation}\label{eq:ODE+BC:w(0)}
w_0\equiv w(0)=-r_0\cdot u(0)-\mathring{u}_0;
\end{equation}
$\mathring{\sigma}(x_0)$ и $\varepsilon(x_0)$ -- 
...
\mathring{\sigma}(x_0)=\intf{x_0}{ x_{1/2} }{ r(x) }
\approx r(x_0)\cdot(x_{1/2}-x_0)=r(x_0)\cdot\dfrac{x_1-x_0}{2},
\end{gather*}
\begin{gather*}%\label{eq:ODE+BC:LEFT:heat source}
\varepsilon(x_0)=\intf{x_0}{ x_{1/2} }{ q(x) }
\approx q(x_0)\cdot(x_{1/2}-x_0)=q(x_0)\cdot\dfrac{x_1-x_0}{2}
\end{gather*}

Из соотношений
\eqref{eq:ODE+BC:LEFT},
\eqref{eq:ODE+BC:w(1/2)} и \eqref{eq:ODE+BC:w(0)} 
следует дискретный аналог краевого условия 
на левой границе рассматриваемой области:
\begin{equation}
-\sigma(x_0)\cdot(u_1-u_0)
+r_0\cdot u_0+\mathring{u}_0
+\mathring{\sigma}(x_0)\cdot u_0
=\varepsilon(x_0)
\end{equation}
...
\begin{equation*}
\begin{cases}
-\sigma(x_0)\cdot(u_1-u_0)-w_0+\mathring{\sigma}(x_0)\cdot u_0=\varepsilon(x_0)\\
-\sigma(x_{i})\cdot(u_{i+1}-u_i) + 
\sigma(x_{i-1})\cdot(u_i-u_{i-1})
+ \mathring{\sigma}(x_i)\cdot u_i = \varepsilon(x_i)\\
w_n+\sigma(x_{n-1})\cdot(u_n-u_{n-1})+\mathring{\sigma}(x_n)\cdot u_n=\varepsilon(x_n)\\
\end{cases}
\end{equation*}
...
&
b_0=\sigma(x_0),&
c_0=b_0+\mathring{\sigma}(x_0),&
y_0=\varepsilon(x_0)+w_0\\
a_i=\sigma(x_{i-1}),&
b_i=\sigma(x_i),&
c_i=a_i+b_i+\mathring{\sigma}(x_i),&
y_i=\varepsilon(x_i)\\
a_n=\sigma(x_{n-1}),&
&
c_n=a_n+\mathring{\sigma}(x_n),&
y_n=\varepsilon(x_n)-w_n\\
\end{matrix}\right.
\end{equation*}
...
($i=1,2,\dots,n-1$) по рекуррентным соотношениям
...
После того как коэффициенты прогонки
найдены, решение системы находится 
...
\begin{figure}[H]\centering
\begin{tikzpicture}[font=\small,>={Straight Barb[angle=45:2mm 1]},shorten >=1pt]
% alpha[1] и beta[1]
\draw[shape=rectangle,minimum height=3em,minimum width=10em]
(0,0) node[thick,draw] (O)
{$\alpha_{1}=\dfrac{b_0}{c_0},\quad\beta_{1}=\dfrac{y_0}{c_0}$};
% начало цикла
\draw[shape=trapezium, minimum height=2em, minimum width=5em]
([yshift=-25mm]O) node[thick,draw] (START CYCLE) {$i=1,2,\ldots,n$};
% стрелка
\draw[->,thick] (O.south) -- (START CYCLE.north);
% alpha[i] и beta[i]
\draw[shape=rectangle, minimum height=4em, minimum width=5em]
([yshift=-45mm]{O}) node[thick,draw] (ABI)
{
$\alpha_{i+1}=\dfrac{b_i}{c_i-a_i\cdot\alpha_{i}}$,\quad
$\beta_{i+1}=\dfrac{a_i\cdot\beta_i+y_i}{c_i-a_i\cdot\alpha_{i}}$
};
% стрелка
\draw[->,thick] (START CYCLE.south) -- (ABI.north);
% конец цикла
\draw[shape=trapezium,minimum height=2em,minimum width=5em,shape border rotate=180]
([yshift=-6em]{START CYCLE.south}) node[thick,draw] (END CYCLE) {конец цикла};
%([yshift=-75mm]O) node[thick,draw] (END CYCLE) {конец цикла};
% стрелка
\draw[->,thick] (ABI.south) -- (END CYCLE.north);
% u[n]
\draw[shape=rectangle, minimum height=3em, minimum width=5em]
([yshift=-4em]{END CYCLE.south}) node[thick,draw] (un)
{$u_n=\dfrac{a_n\cdot\beta_n+y_n}{c_n-a_n\cdot\alpha_n}$};
% стрелка
\draw[->,thick] (END CYCLE.south) -- (un.north);
% начало цикла
\draw[shape=trapezium,minimum height=3em,minimum width=3em]
([yshift=-4em]un.south) node[thick,draw] (START CYCLE) {$i=(n-1),\ldots,1,0$};
% u[i]
\draw[shape=rectangle,minimum height=3em,minimum width=6em]
([yshift=-3em]{START CYCLE.south}) node[thick,draw] (UI)
{$u_i=\alpha_{i+1}\cdot u_{i+1} + \beta_{i+1}$};
% конец цикла
\draw[shape=trapezium,shape border rotate=180,minimum height=3em,minimum width=6em]
([yshift=-0em]{START CYCLE.south}) node[thick,draw] (END CYCLE) {конец цикла};

\end{tikzpicture}
% *******************************************
\caption{Схема вычислений неизвестных краевой задачи\linebreak
методом прогонки}
\label{fig:ODE+BC:scheme shuttle}
\end{figure}
...
\begin{equation*}
\left\{\begin{matrix}[rcl]
w_{i+1/2}&=&-\sigma(x_i)\cdot(u_{i+1}-u_i)\\
u_i&=&\alpha_{i+1}\cdot u_{i+1} + \beta_{i+1}\\
\end{matrix}\right.
\end{equation*}
...
\begin{equation}
\bar{\alpha}_{i+1}=\dfrac{\alpha_{i+1}}{\alpha_{i+1}-1}\cdot\sigma^{-1}(x_i),
\quad
...
\begin{equation}
w_{i-1/2}=\left(1+\bar{\alpha}_{i+1}\cdot\mathring{\sigma}(x_i)\right)
\cdot w_{i+1/2}
+ \bar{\beta}_{i+1}\cdot\mathring{\sigma}(x_i)
- \varepsilon(x_i)\\
\end{equation}
...
\node[on chain,rectnode]
{$\alpha_{1}=\dfrac{b_0}{c_0},\quad\beta_{1}=\dfrac{y_0}{c_0}$};
% alpha[i+1], beta[i+1]
\node[pointnode,on chain=going left]{};
\begin{scope}[every node/.style={fill=darkblue!10}]
\node[cyclenode,on chain,join,continue chain=going below] (SC)
{$i=1,2,\ldots,n$};
\node[on chain,rectnode,join] (ABI)
{$\alpha_{i+1}=\dfrac{b_i}{c_i-a_i\cdot\alpha_{i}}$,\quad
$\beta_{i+1}=\dfrac{a_i\cdot\beta_i+y_i}{c_i-a_i\cdot\alpha_{i}}$};
% стрелка
\draw[->] (ABI.east) -- ++(10mm,0mm) |- (SC.east);
\end{scope}
\node[rectnode,on chain,join]
{$u_n=\dfrac{a_n\cdot\beta_n+y_n}{c_n-a_n\cdot\alpha_n}$};
% u[i]
\begin{scope}[every node/.style={fill=darkred!10}]
\node[cyclenode,on chain,join,continue chain=going below] (SC)
{$i=(n-1),\ldots,1,0$};
\node[rectnode,on chain,join] (UI)
{$u_i=\alpha_{i+1}\cdot u_{i+1} + \beta_{i+1}$};
% стрелка
\draw[->] (UI.east) -- ++(10mm,0mm) |- (SC.east);
\end{scope}
\draw[->] (SC.west) -- ++(-10mm,0mm) -- +(0mm,-30mm);
\end{tikzpicture}
\caption{Блок-схема метода прогонки}
\end{figure}
