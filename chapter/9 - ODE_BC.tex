...
% alpha[1] и beta[1]
\node[rectnode,on chain]
{$\alpha_{1}=\dfrac{b_0}{c_0},\quad\beta_{1}=\dfrac{y_0}{c_0}$};
% начало цикла
\node[startcyclenode,on chain,join]{$i=1,2,\ldots,n$};
% alpha[i] и beta[i]
...
% конец цикла
\node[endcyclenode,on chain,join]{конец цикла};
%
\node[circlenode,on chain,join]{1};
...
\node[rectnode,on chain,join,continue chain=going below] (SC)
{$i=1,2,\ldots,n$};
\node[rectnode,on chain,join] (ABI)
{$\alpha_{i+1}=\dfrac{b_i}{c_i-a_i\cdot\alpha_{i}}$,\quad
$\beta_{i+1}=\dfrac{a_i\cdot\beta_i+y_i}{c_i-a_i\cdot\alpha_{i}}$};
% стрелка
...
\node[startcyclenode,on chain,join,continue chain=going below] (SC)
{$i=(n-1),\ldots,1,0$};
\node[rectnode,on chain,join] (UI)
{$u_i=\alpha_{i+1}\cdot u_{i+1} + \beta_{i+1}$};
\node[endcyclenode,on chain,join]
{конец цикла};
% стрелка
