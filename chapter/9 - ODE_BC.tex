...
% разрыв функции
\addplot[mark=*,draw=darkred,thick,mark size=3pt,mark options={fill=white}]
coordinates{(0,30)(30,30)(50,30)};
\path (axis cs:50,30) -- (axis cs:50,70) 
node[midway,sloped,darkred,fill=white]{разрыв};
\addplot[mark=*,draw=darkred,thick,mark size=3pt,mark options={fill=white}]
coordinates{(50,70)(70,70)(100,70)};
\end{axis}
\end{tikzpicture}
...
\begin{subfigure}[b]{.5\linewidth}\centering
\begin{tikzpicture}
\begin{axis}[uxwx axis style,%xlabel=$u(x)$,
ytick=data,yticklabels={$u_{i-1}$,$u_i$,$u_{i+1}$}]
\addplot[ball darkblue,thick]
coordinates{(0,80)(50,30)(100,50)};
\end{axis}
\end{tikzpicture}
\caption{температура $u(x)$}
\label{fig:ODE+BC:temperature:u(x)}
\end{subfigure}%
%
% второй график
%
...
ytick=data,yticklabels={$w_{i-1/2}$,$w_{i+1/2}$}]
\addplot[ball darkred,thick,const plot mark mid]
coordinates{(30,10)(75,70)};
\end{axis}
\end{tikzpicture}
\caption{тепловой поток}
\label{fig:ODE+BC:heat flow:w(x)}
\end{subfigure}
\caption{
