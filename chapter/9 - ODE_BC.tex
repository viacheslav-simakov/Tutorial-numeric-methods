...
w(0)&+&r_0\cdot u(0)&=&s_0\\
-w(\ell)&+&r_{\ell}\cdot u(\ell)&=&s_{\ell}
\end{matrix}\right.,
\end{gather}
...
w_0\equiv w(0)=s_0-r_0\cdot u(0);
\end{equation}
...
w_n\equiv w(\ell)=r_{\ell}\cdot u(\ell)-s_{\ell};
\end{equation}
$w_{n-1/2}$ -- тепловой поток в последнем 
промежуточном узле $x_{n-1/2}$:
\begin{equation}
w_{n-1/2}=-\sigma(x_{n-1})\cdot(u_n-u_{n-1});
\end{equation}
...
\begin{equation*}
\left\{\begin{matrix}[rcl]
-\sigma(x_0)\cdot(u_1-u_0)-(s_0-r_0\cdot u_0)
...
+ \mathring{\sigma}(x_i)\cdot u_i
&=&\varepsilon(x_i)\\
(r_{\ell}\cdot u_n-s_{\ell})+\sigma(x_{n-1})\cdot(u_n-u_{n-1})+
\mathring{\sigma}(x_n)\cdot u_n
&=&\varepsilon(x_n)\\
\end{matrix}\right.
\end{equation*}
...
&
b_0=\sigma(x_0),&
c_0=b_0+\mathring{\sigma}(x_0)+r_0,&
y_0=\varepsilon(x_0)+s_0\\
a_i=\sigma(x_{i-1}),&
b_i=\sigma(x_i),&
...
a_n=\sigma(x_{n-1}),&
&
c_n=a_n+\mathring{\sigma}(x_n)+r_{\ell},&
y_n=\varepsilon(x_n)+s_{\ell}\\
\end{matrix}\right.
\end{equation*}
