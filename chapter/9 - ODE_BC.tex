...
чтобы точки разрыва коэффициентов $f(x)=\{k(x), r(x), q(x)\}$ 
...
%
\begin{figure}[H]\centering
\begin{tikzpicture}[>={Straight Barb[angle=45:1.5mm 1]},shorten >=3pt,shorten <=3pt]% стрелка
\begin{axis}[xlabel=\empty,
xmin=-10,xmax=110,ymin=10,ymax=90,
xtick={0,30,50,70,100},xticklabels={$0$,$x_{i-1}$,$x_i$,$x_{i+1}$,$\ell$},
ytick={30,70},yticklabels={$f_1$,$f_2$}]
% разрыв функции
\addplot[ball darkgreen,thick] coordinates{(0,45)(30,35)(50,30)};
\draw[<->,thin,darkgreen] (axis cs:50,30) -- (axis cs:50,70) 
node[midway,sloped,fill=white]{разрыв};
\addplot[ball darkgreen,thick] coordinates{(50,70)(70,60)(100,55)};
\end{axis}
\end{tikzpicture}
...
axis y line*=left,
ymin=0,ymax=100,ytick=data,
yticklabels={$u_{i-1}$,$u_i$,$u_{i+1}$}]
\addplot[ball darkblue,thick]
coordinates{(0,80)(50,30)(100,50)};
\addplot[draw=none,fill=blue!5,on layer=background]
coordinates{(0,80)(50,30)(100,50)}\closedcycle;
\end{axis}
% правая ось OY
\begin{axis}[uxwx axis style,
axis y line*=right,axis x line=none,grid=none,
ymin=-30,ymax=50,ytick={-20,30},
yticklabels={$w_{i-1/2}$,$w_{i+1/2}$}]
\addplot[draw=darkred,thick] 
coordinates{(0,5)(30,-15)(75,30)(100,40)};
\addplot[ball darkred,only marks] coordinates{(30,-15)(75,30)};
\end{axis}
\end{tikzpicture}
\caption{
...
u(x_i)=\mathrm{const},\quad&x\in(x_{i-1/2},x_{i+1/2}) \\
w(x_{i+1/2})=\mathrm{const},\quad&x\in(x_i,x_{i+1})\\
k(x_i), r(x_i), q(x_i)=\mathrm{const},\quad&x\in(x_i,x_{i+1})
\end{array}\right.
\end{gather}
