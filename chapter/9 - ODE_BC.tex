...
([yshift=-4em]O.south) node[thick,draw] (START CYCLE) {$i=1,2,\ldots,n$};
% стрелка
\draw[->,thick] (O.south) -- (START CYCLE.north);
% alpha[i] и beta[i]
\draw[shape=rectangle, minimum height=4em, minimum width=5em]
([yshift=-3em]{START CYCLE.south}) node[thick,draw] (ABI)
{
$\alpha_{i+1}=\dfrac{b_i}{c_i-a_i\cdot\alpha_{i}}$,\quad
$\beta_{i+1}=\dfrac{a_i\cdot\beta_i+y_i}{c_i-a_i\cdot\alpha_{i}}$
};
...
([yshift=-8em]{START CYCLE.south}) node[thick,draw] (END CYCLE) {конец цикла};
% стрелка
