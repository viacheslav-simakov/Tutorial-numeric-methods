...
% начало цикла
\draw[shape=trapezium, minimum height=2em, minimum width=10em]
([yshift=-4em]un.south) node[thick,draw] (START CYCLE) {$i=(n-1),\ldots,1,0$};
% u[i]
\draw[shape=rectangle, minimum height=3em, minimum width=7em]
([yshift=-3em]{START CYCLE.south}) node[thick,draw] (UI)
{$u_i=\alpha_{i+1}\cdot u_{i+1} + \beta_{i+1}$};
% конец цикла
\draw[shape=trapezium,minimum height=2em,minimum width=10em,shape border rotate=180]
([yshift=-6em]{START CYCLE.south}) node[thick,draw] (END CYCLE) {конец цикла};
