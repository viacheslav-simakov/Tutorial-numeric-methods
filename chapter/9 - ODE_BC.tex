...
\begin{equation*}
\sigma(x_i)=\dfrac{2\cdot k(x_i)\,k(x_{i+1})}{k(x_i)+k(x_{i+1})}
...
\node[rrectnode,on chain,join]{старт};
% входные данные
\node[datanode,on chain,join]{$\{x_i\},p_0,s_0,p_{\ell},s_{\ell}$};
% sigma[0] и varepsoln[0]
\node[rectnode,on chain,join]{$
\mathring{\sigma}_0=r_0\cdot\dfrac{x_1-x_0}{2},
\quad
\varepsilon_0=q_0\cdot\dfrac{x_1-x_0}{2}
$};
% c[0]
\node[rectnode,on chain,join]{$
b_0=\sigma_0,\lambda_0
\quad
c_0=b_0+\mathring{\sigma}_0+p_0,
\quad
...
\node[rectnode,on chain,join]
{$\lambda_i=
2\cdot\dfrac{k_i\cdot k_{i+1}}{k_i+k_{i+1}}\cdot
\dfrac{1}{x_{i+1}-x_i}
$};
% sigma[i] - конвекция
\node[rectnode,on chain,join]
...
\node[rectnode,on chain,join]
{$\varepsilon_i=
q(x_i)\cdot\dfrac{x_i-x_{i-1}}{2}+
q(x_{i+1})\cdot\dfrac{x_{i+1}-x_i}{2}
$};
% конец цикла
\node[endloopnode,on chain,join]{$i=0$};
% выходные данные
\node[datanode,on chain,join]{$\{a_i\},\{c_i\},\{b_i\},\{y_i\}$};
% стоп
\node[rrectnode,on chain,join]{стоп};
\end{tikzpicture}
\caption{Блок-схема алгоритма вычислений\linebreak
коэффициентов $\{a_i\},\{c_i\},\{b_i\},\{y_i\}$
системы линейных уравнений}
\label{fig:ODE+BC:a+b+c+y}
\end{figure}
...
краевой задачи во всех узлах сетки $\{x_i\}$.
\item
Определить коэффициенты системы линейной уравнений
разностной схемы $\{a_i\}$, $\{b_i\}$, $\{c_i\}$ и $\{y_i\}$.
\item
...
методом прогонки (рисунок \ref{fig:ODE+BC:scheme shuttle:u(x)}).
\item
...
% входные данные
\node[datanode,on chain,join]{%
$s_0,p_0,\{\mathring{\sigma}_i\},\{\varepsilon_i\},\{u_i\},$};
% w[0]
\node[rectnode,on chain,join]{$
w_0=s_0-p_0\cdot u_0
$};
% w[1/2]
\node[rectnode,on chain,join]{$
w_{1/2}=w_0-\mathring{\sigma}_0\cdot u_0+\varepsilon_0
$};
% начало цикла
\node[startloopnode,on chain,join]{$i=1,2,\ldots,(n-1)$};
% w[i+1/2]
...
% w[n]
\node[rectnode,on chain,join]{$
w_n=w_{n-1/2}-\mathring{\sigma}_n\cdot u_n + \varepsilon_n$};
% выходные данные
