...
% выходные данные
\node[datanode,on chain,join]{$\{w_i\}$};
% стоп
\node[rrectnode,on chain,join]{стоп};
\end{tikzpicture}
\caption{Блок-схема вычислений потока неизвестной функции
$\{w_i\}=(w_0,w_{1/2},w_{3/2},\ldots,w_{n-1/2},w_n)$ 
в промежуточных узлах 
расчетной сетки $\{x_{i\pm1/2}\}=(x_0,x_{1/2},x_{3/2},\ldots,x_{n-1/2},x_n)$}
