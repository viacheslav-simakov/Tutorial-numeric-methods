...
%
\emptyline
\subsection{Интервальный метод}
Интервальный метод поиска корня уравнения $f(x)=0$ 
состоит следующем:
...
\end{gather*}
и с помощью аналогичной процедуры (1) процесс поиска корня уравнения $f(x)=0$ 
...
\draw[darkblue] (axis cs:1.2,-0.5) node[below left] {$0>f(x_{i+1})$};
%
\path [name path=B] (axis cs: 0,-2) -- (axis cs: 1.75,-2);
\addplot[orange!15] fill between [of=A and B, soft clip={domain=0.75:1.25},];
\end{axis}
\end{tikzpicture}
\caption{Иллюстрация интервального метода поиска корня уравнения}
\end{figure}
...
\end{axis}
\end{tikzpicture}
\caption{График функции $f(x)$ (сплошная линия) и 
