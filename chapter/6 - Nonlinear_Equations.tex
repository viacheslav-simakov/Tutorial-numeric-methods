...
[
	...
	xmin=-1.5, xmax=2.75, xtick={-1,0,1,1.5,2}, xticklabels={$x_1$,$\vec a$,$x_2$,$\vec b$,$x_3$},
...
\addplot[very thick, color=black,domain=-1.5:2.75, samples=2] {0};
\addplot[very thick, color=red,domain=-1.25:2.5, samples=100] {(x-1)*(x+1)*(x-2)};
...
$x_1$, $x_2$ и $x_3$ -- действительные корни уравнения $f(x)=0$,\\ т.е. $f(x_1)=0, f(x_2)=0, f(x_3)=0,$
\end{center}
...
и проводится их разделение, т. е. \textit{выделяются области} в комплексной плоскости, 
\textit{содержащие только один корень}.
Кроме того, изучается вопрос о кратности корней.
Тем самым находятся некоторые начальные приближения для корней уравнения. 
\item
На втором этапе, \textit{используя заданное начальное приближение},
строится итерационный процесс, позволяющий \textit{уточнить значение отыскиваемого корня}.
\end{enumerate}

\begin{tcolorbox}[width=\textwidth,colback={blue!10}, boxrule=0mm, arc=0mm]
...
\addplot[very thick, color=black,domain=-0.5:2.15, samples=2] {0};
\addplot[name path=A,very thick, color=black,domain=-0.35:1.95, samples=100] {(x-1)*(x+1)*(x-2)};
\addplot[only marks, mark=*, mark size=4pt, mark options={fill=red!75, draw=black, solid}]
coordinates {(0,2) (0.25,1.640625) (0.5,1.125) (0.75,0.546875)};
\draw[red] (axis cs:0.72,0.47) node[left] {$f(x_i)>0$};
\addplot[only marks, mark=*, mark size=4pt, mark options={fill=blue!75, draw=black, solid}]
coordinates {(1.25,-0.421875) (1.5,-0.625) (1.75,-0.515625)};
\draw[blue] (axis cs:1.2,-0.5) node[left] {$f(x_{i+1})<0$};
%
\path [name path=B] (axis cs: 0,-2) -- (axis cs: 1.75,-2);
\addplot [orange!20] fill between [of=A and B, soft clip={domain=0:1.75}];
\addplot [olive!10] fill between [of=A and B, soft clip={domain=0.75:1.25},];
\end{axis}
\end{tikzpicture}
