...
xlabel=$x$,ylabel=$f(x)$,
%xmin=-1,xmax=1.5,xtick={-0.5,1},xticklabels={$x_{i-1}$,$x_i$},
%ymin=-0.25,ymax=1.5,ytick={0,1,0.25},yticklabels={0,$y_{i-1}$,$y_i$},
%ymin=-0.25,
width=8cm,
% определение функции
declare function={
...
#1
% L0(x)
\addplot[name path=INT,
draw=black,thin,fill=darkred!25,opacity=0.25,
domain=-0.5:2,samples=2]{1} \closedcycle;
\end{axis}
\end{tikzpicture}
}
%
...
\FigInt{
\addplot[name path=F,fill=gray!20,
mark=ball,mark size=3pt,mark indices={1,50},
mark options={ball color=red!50,thin,draw=black},
draw=darkred,thick,samples=50,domain=-0.5:2]{cubic(x)}
node[pos=0.5,above right,color=darkred]{$f(x)$} \closedcycle;
}
%
\hskip 10pt
%
...
width=8cm]
% график функции
\draw[darkred,dashed] (axis cs: -0.5,0) -- (axis cs: -0.5,1);
\addplot[name path=Func,
darkred,very thick,samples=50,domain=-0.5:1]{1.25-x^2}
node[pos=0.5,above right] {$f(x)$};
\draw[darkred,dashed] (axis cs: 1,0.25) -- (axis cs: 1,0);
% ось x
\draw[darkred,dashed,name path=Ox] (axis cs: -0.5,0) -- (axis cs: 1,0);
% L0(x)
\path[name path=INT] (axis cs: -0.5,0.25) -- (axis cs: 1,0.25);
% левый прямоугольник
\addplot [gray!15] fill between [of=Ox and INT, soft clip={domain=-0.5:1}];
\end{axis}
\end{tikzpicture}
\end{figure}
