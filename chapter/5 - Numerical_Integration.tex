...
Если функция $f(x)$ непрерывна на отрезке $x\in[a,b]$ и известна ее 
первообразная $F(x)$, то определенный интеграл от этой функции 
в пределах от $a$ до $b$ может быть вычислен по формуле Ньютона -- Лейбница:
\begin{gather*}
\int\limits_{a}^{b}f(x)dx=F(b)-F(a)
,\end{gather*}
где $F^{\prime}(x)=f(x)$ -- первообразная подынтегральной функции $f(x)$.

Однако во многих случаях первообразная функция $F(x)$ не может 
быть найдена с помощью элементарных средств или является  
слишком сложной, поэтому вычисление определенного интеграла 
...
\textit{Задача} численного интегрирования функции заключается в  
...
функцией $\varphi(x)$ простого вида (например, полиномом), а затем приближенно полагают:
\begin{gather*}
\int\limits_{a}^{b}f(x)dx\approx \int\limits_{a}^{b}\varphi(x)dx
\end{gather*}

Далее рассматриваются способы приближенного вычисления определенных интегралов вида:
\begin{gather*}
...
$x_i$ -- узлы квадратурной формулы, т.е. точки отрезка $[a, b], (i= 0,1,\cdots,n)$.
...
Для построения формулы численного интегрирования 
на всем отрезке $[a,b]$ достаточно построить квадратурную формулу 
на частичном отрезке $x\in[x_{i-1},x_i]$ для интеграла:
\begin{gather*}
I_i=\int\limits_{x_{i-1}}^{x_i}f(x)dx
\end{gather*}

% ------------------------------------------------------------------
\subsection{Формула трапеций}

На частичном отрезке квадратурная \textit{формула трапеций} 
является следствием замены подынтегральной функции $f(x)$
интерполяционным полиномом Лагранжа первой степени 
$f(x)\approx L_1(x)$, построенным по множеству узлов
сетки $\{x_{i-1}, x_i\}$:
\begin{gather*}
L_1(x)=\dfrac{x-x_i}{x_{i-1}-x_i}\cdot y_{i-1} + \dfrac{x-x_{i-1}}{x_i-x_{i-1}}\cdot y_i=
\dfrac{(x-x_{i-1})\cdot y_i-(x-x_i)\cdot y_{i-1}}{h_i}
,\end{gather*}
где $h_i=(x_i-x_{i-1})$ -- расстояние между соседними узлами интерполяции,
т.е. шаг неравномерной сетки; $y_i=f(x_i)$ -- значение функции в узлах интерполяции.

Интегрирование интерполяционного полинома Лагранжа на частичном отрезке
определяет формулу трапеций:
\begin{gather*}
I_i\approx\int\limits_{x_{i-1}}^{x_i}\dfrac{(x-x_{i-1})\cdot y_i-(x-x_i)\cdot y_{i-1}}{h_i}dx=
\dfrac{y_{i-1}+y_{i}}{2}\cdot h_{i}
\end{gather*}

Таким образом, квадратурная формула трапеций имеет вид:
\begin{gather*}
I\approx\sum\limits_{i=1}^{N}\dfrac{y_{i-1}+y_{i}}{2}\cdot h_{i}
,\end{gather*}
где $N$ -- количество узлов интерполяции.

% ------------------------------------------------------------------
\subsection{Формула Симпсона}
На частичном отрезке квадратурная \textit{формула Симпсона} 
является следствием аппроксимации на частичном отрезке 
подынтегральной функции $f(x)$ интерполяционным полиномом
Лагранжа второй степени $f(x)\approx L_2(x)$, который построен
по узлам сетки $\{x_{i-1}, x_{i}, x_{i+1}\}$:
\begin{equation*}
\begin{matrix}
	L_{2}(x)&=&\dfrac
...
\begin{gather*}
I_i\approx\int\limits_{x_{i-1}}^{x_{i+1}}L_2(x)dx=
\int\limits_{x_{i-1}}^{x_{i+1}}\left(a_i x^2+b_i x + c_i\right)dx
,\end{gather*}
где $a_i$, $b_i$, $c_i$ -- коэффициенты при соответствующих степенях $x$
интерполяционного полинома Лагранжа $L_2(x)$
в пределах частичного отрезка $x\in[x_{i-1}, x_{i+1}]$.

Таким образом, квадратурная формула Симпсона имеет вид:
\begin{gather*}
I\approx\sum\limits_{i=1,3,5,\cdots}^{N-1}
\left[
{a_i}\dfrac{x_{i+1}^3-x_{i-1}^3}{3}+
{b_i}\dfrac{x_{i+1}^2-x_{i-1}^2}{2}+
{c_i}(x_{i+1}-x_{i-1})
\right]
,\end{gather*}
где $N$ -- количество узлов интерполяции.
...
На множестве узлов сетки $\{x_i\}$ определены значения некоторой функции $\{f(x_i)\}$:
\begin{center}
\begin{tabular}{l *{5}{l}}
\toprule
