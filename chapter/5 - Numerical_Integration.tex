...
и осью абсцисс $Ox$ (выделенная область на графике).
...
% f(x)
\addplot[name path=A,draw=darkblue,thick,
...
node[pos=0.7,color=darkblue,opacity=1,above] {$L_0(x)$}
}
%
\hskip 10pt
%
\FigInt{-0.7}{2}{ {cubic(2)}
node[pos=0.8,color=darkblue,opacity=1,above] {$L_0(x)$}
}
...
node[pos=0.8,color=darkblue,opacity=1,above] {$L_1(x)$}
}
\caption{График подынтегральной функции $f(x)$
...
\begin{figure}[H]\centering
\FigInt{-0.7}{2}{ {Lagrange(x)} 
node[pos=0.7,color=darkblue,opacity=1,above right] {$L_2(x)$}
}
...
\begin{axis}
\addplot[name path=A,
smooth,thick,color=darkblue,mark=ball,mark size=3pt,
...
заданной таблично на отрезке $x\in[-3.31, 3.50]$.
...
\begin{gather*}
\begin{matrix}
S_1=&\dfrac{f(x_0)+f(x_1)}{2}\cdot h_1=\dfrac{2.45+4.03}{2}\cdot3.62=11.73\\[1em]
S_2=&\dfrac{f(x_1)+f(x_2)}{2}\cdot h_2=\dfrac{4.03-3.61}{2}\cdot1.01=0.21\\[1em]
S_3=&\dfrac{f(x_2)+f(x_3)}{2}\cdot h_3=\dfrac{-3.61+4.50}{2}\cdot1.15=0.51\\[1em]
S_4=&\dfrac{f(x_3)+f(x_4)}{2}\cdot h_4=\dfrac{4.50+3.10}{2}\cdot1.03=3.91\\
\end{matrix}
\end{gather*}

Определим интеграл $I$ на всем отрезке интегрирования 
$[-3.31, 3.50]$, воспользовавшись свойством аддитивности 
интеграла:
\begin{gather*}
...
\begin{tikzpicture}
\begin{axis}[ymax=10]
\addplot[only marks,draw=darkblue,mark=ball,mark size=3pt,
mark options={ball color=darkblue!50,thin,draw=darkblue}
] coordinates 
{(-3.31,2.45) (0.31,4.03) (1.32,-3.61) (2.47,4.50) (3.50, 3.1)};
%\path [name path=B] (\pgfkeysvalueof{/pgfplots/xmin},0) -- (\pgfkeysvalueof{/pgfplots/xmax},0);
\addplot[name path=A,thick,color=darkblue,domain=-3.31:1.32]
{-1.73*x^2-4.74*x+5.66} node[pos=0.4,right] {$L^{(1)}_2(x)$};
