...
\newcommand\intfleft[1]{ \intf{ x_{i-\onehalf} }{ x_{i} }{#1} }
\newcommand\intfCenter[1]{ \intf{ x_{i-\onehalf} }{ x_{i+\onehalf} }{#1} }
\newcommand\intfright[1]{ \intf{ x_{i} }{ x_{i+\onehalf} }{#1} }
...
\intf{ x_i }{ x_{i\pm1} }{ u^{\prime}(x) }
=-\intf{ x_i }{ x_{i\pm1} }{ \dfrac{w(x)}{k(x)} }
\eqsep
\label{eq:ODE_BC(FourierLaw)}
u(x_{i\pm1})-u({x_{i}})=-\intf{ x_i }{ x_{i\pm1} }{ \dfrac{w(x)}{k(x)} }
.\end{gather}
...
w(x_{i\pm\onehalf})=\mathrm{const},\quad&x\in(x_i,x_{i+1})\\
...
\eqsep
u(x_{i})-u(x_{i-1})&\approx&-\dfrac{w(x_{i-\onehalf})}{k_{i}}\cdot h_{i}
\end{array}
,\end{gather*}
и найдем поток тепла в промежуточных узлах сетки $w(x_{i\pm\onehalf})$:
\begin{gather}\label{eq:ODE_BC(w)}
\begin{array}{rcl}
w(x_{i+\onehalf})&\approx&-{k}_{i+1}\cdot\dfrac{u(x_{i+1})-u(x_{i})}{h_{i+1}}
\eqsep
w(x_{i-\onehalf})&\approx&-{k}_{i}\cdot\dfrac{u(x_{i})-u(x_{i-1})}{h_{i}}
\end{array}
\end{gather}
где $k_{i}=k(x_{i})$ -- значение коэффициента 
температуропроводности в узлах расчётной сетки $\{x_{i}\}$.
...
\eqref{eq:ODE_BC(<g>)}, можно записать в форме разностного уравнения, 
содержащего значения искомой функции $u(x)$
в узлах сетки $\{x_{i-1}$, $x_i$, $x_{i+1}\}$:
\begin{gather}\label{eq:ODE_BC(FD)}
-k_{i+1}\cdot\dfrac{u(x_{i+1})-u(x_{i})}{h_{i+1}}
+k_i\cdot\dfrac{u(x_{i})-u(x_{i-1})}{h_i}
+\overline{r}_i\cdot u(x_i)=\overline{g}_i
\end{gather} 
 
\begin{tcolorbox}%[colback=blue!10]
Уравнение \eqref{eq:ODE_BC(FD)} по своему построению является разностным аналогом 
...
относительно $N+1$ неизвестных $u(0), u(x_1),\dots, u(x_i),\dots, u(x_{N-1}),u(\ell)$.
...
\begin{gather*}
\intf{0}{ x_{\onehalf} }{ w^{\prime}(x) }
+\intf{0}{ x_{\onehalf} }{ r(x)u(x) }
=\intf{0}{ x_{\onehalf} }{ g(x) }
\end{gather*}

Рассуждая аналогично вышесказанному, запишем уравнение 
...
,\end{gather*}
где $w(x_{\onehalf})$ -- тепловой поток в первом промежуточном узле $x_{\onehalf}$
определяется из краевого условия на левой границе стержня \eqref{eq:ODE_BC(w)}:
\begin{gather*}
w(x_{\onehalf})=-k_{1}\cdot\dfrac{u(x_{1})-u(0)}{h_{1}}
,\end{gather*}
...
\overline{r}_0=\intf{0}{ \nicefrac{h_1}{2} }{ r(x) }=\dfrac{r_0h_1}{2},
\quad%\eqsep
\overline{g}_0=\intf{0}{ \nicefrac{h_1}{2} }{ g(x) }=\dfrac{g_0h_1}{2},
\end{gather}
...
-k_1\cdot\dfrac{u(x_{1})-u(0)}{h_{1}}
-\brackets{ \mu_1-\lambda_1 u(0) }
+\overline{r}_0 u(0)=\overline{g}_0
,\end{gather}
...
%
Для определения коэффициентов разностной схемы
