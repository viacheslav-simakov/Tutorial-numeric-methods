...
% функция f(x)
\addplot[thick,color=darkred,domain=0:1.5, samples=50] {(x-1)^2};
\addplot[thick,color=darkred,domain=1.5:3, samples=50] {1+ln(x-0.75)};
% маркеры
...
из прилегающих к точке $x=x_1$ отрезков, то есть
минимум находится либо в пределах отрезка $[a,x_1]$,
либо в $[x_1,x_2]$.
...
то есть область поиска минимума функции сужается:
\begin{equation*}
\abs{a-x_2}<\abs{a-b},\quad\text{так как}\quad x_2<b.
\end{equation*} 

На отрезке $[a, x_2]$ вновь необходимо выбрать две 
внутренние точки, вычислить в них и на концах отрезка 
значения функции, и сделать следующий шаг итерационного процесса. 

Практически на каждом шаге выбирается две точки $x_1$ и $x_2$
внутри отрезка $[a,b]$, равноудаленные от концов этого отрезка.
...
$\abs{x_k-x_{k-1}}$ была менее $\epsilon$, 
должна выполняться оценка числа итераций $k$:
\begin{equation*}
\ell_k=\left(\dfrac{2}{3}\right)^k\cdot(b-a)\leqslant\epsilon
\end{equation*}
