...
имеет в декартовой системе координат компоненты:
\begin{equation*}
\vect{r}=\left(
...
Кратчайшее расстояние между двумя точками $P_1$ и $P_2$
...
$P_1(\phi_1,\lambda_1)$ и $P_2(\phi_2,\lambda_2)$, то
расстояние на поверхности Земли между этими точками
составляет (рисунок \ref{fig:GCS:creat circle distance}):
\begin{equation}
d_{12}(\phi_1,\lambda_1,\phi_2,\lambda_2)=\mathcal{R}\cdot\alpha_{12},
\end{equation}
где $\mathcal{R}$ -- радиус планеты Земля;
$\alpha_{12}$ -- центральный угол между радиус-\linebreak
векторами $\vect{r}_1$ и $\vect{r}_2$ 
точек $P_1$ и $P_2$, лежащих на поверхности Земли,
который можно определить из выражения 
для скалярного произведения этих векторов:
\begin{equation}
(\vect{r}_1,\vect{r}_2) = \norma{\vect{r}_1}\cdot\norma{\vect{r}_2}
\cdot\cos(\alpha_{12})
\quad\to\quad
\alpha_{12}=\arccos\left(
\dfrac{(\vect{r}_1,\vect{r}_2)}{\norma{\vect{r}_1}\cdot\norma{\vect{r}_2}}
\right),
%d(\phi_1,\lambda_1,\phi_2,\lambda_2)=
%R\cdot\mathrm{acos}(
%\sin(\phi_1)\sin(\phi_2)+
%\cos(\phi_1)\cos(\phi_2)\cdot\cos(\lambda_2-\lambda_1)
\end{equation}
...
% точки на сфере 
\draw[fill=red] (A) circle (0.5pt) node[below right]{$P_1$};
\draw[fill=blue] (B) circle (0.5pt) node[above left]{$P_2$};
% радиус-вектор
...
между точками $P_1$ и $P_2$}
\label{fig:GCS:creat circle distance}
\end{figure}

Скалярное произведение векторов -- это операция 
над двумя векторами $\vect{a}$ и $\vect{b}$, 
результатом которой является число скаляр, 
не зависящее от системы координат, то есть инвариант:
...
в ортонормированной системе координат, например, в декартовой.

Компоненты радиус-векторов 
$\vect{r}_1$ и $\vect{r}_2$ точек $P_1$ и $P_2$
