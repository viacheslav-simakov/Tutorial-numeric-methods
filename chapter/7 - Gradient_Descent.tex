...
% начало цикла
\node[startloopnode,on chain,join](loop){$i=1,2,3,\ldots$};
% фиксирование переменных
\node[rectnode,on chain,join]
{$x_j=\const,
\quad\forall j \ne i,
\quad j=1,2,\ldots,n$};
% min
\node[rectnode,on chain,join](min)
...
% выход f
\draw[->](compare-f.west) -- ([xshift=-15mm]compare-f.west) |- (stop.west);
\end{scope}
\end{tikzpicture}
% *******************************************
\caption{Блок-схема алгоритма нахождения минимума\linebreak 
