...
enlargelimits=true,
xmin=0, xmax=3, xtick={0,0.5,2,3}, xticklabels={$a$,$x_1$,$x_2$,$b$},
ymin=0, ymax=2, ytick={\empty},%ytick={1,{0.5^2},{1+ln(2-0.75)}}, yticklabels={},%{$f(a)$,$f(x_1)$,$f(x_2)$},
]
% функция f(x)
...
node[above]{$f(b)$};
\addplot[only marks,ball darkblue] coordinates{(0.5,{0.5^2})}
node[above right]{$f(x_1)$};
\addplot[only marks,ball darkblue] coordinates{(2,{1+ln(2-0.75)})}
