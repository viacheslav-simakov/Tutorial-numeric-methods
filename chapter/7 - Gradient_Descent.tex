...
\item
В качестве критерия прекращения итерационного поиска
минимума целевой функции выберем точность определения
местоположения склада на текущем шаге итерационного процесса:
\begin{equation}\label{eq:GCS:stop error}
\sqrt{(\phi_k-\phi_{k-1})^2+(\lambda_k-\lambda_{k-1})^2}
\leqslant\epsilon_1=10^{-2},
\end{equation}
где $\phi_k$ и $\lambda_k$ -- значение географических координат
склада на $k$-ом шаге итерационного процесса.

\item
В качестве нулевого приближения выберем границу области поиска
минимума целевой функции:
\begin{equation*}
\begin{cases}
\phi_0 = \phi_{\mathrm{start}}\\
\lambda_0 = \lambda_{\mathrm{end}}\\
\end{cases}
\quad\to\quad
\begin{cases}
\phi_0 = 56.671\\
...
\begin{equation*}
g(\phi)=r(\phi, \lambda_0)=r(\phi, 46.031)
\end{equation*}

\item
Находим минимум функции одной переменной,
