...
нулевого меридиана;
$\vect{r}$ -- радиус-вектор точки $A$, находящейся 
на поверхности Земли радиуса $\mathcal{R}$, 
имеющий в декартовой системе координат компоненты:
\begin{equation*}
\vect{r}=\left(
\mathcal{R}\cdot\cos(\phi)\cos(\lambda),
\mathcal{R}\cdot\cos(\phi)\sin(\lambda),
\mathcal{R}\cdot\sin(\phi)
\right)
\end{equation*}
}
\end{figure}
...
% дуга
\draw[darkblue,thick,tdplot_main_coords]
(A) to [bend right=50]node[sloped,pos=0.5,above]{$d_{12}$} (B);
% точки на сфере 
\draw[fill=red] (A) circle (0.5pt) node[below right]{$1$};
\draw[fill=blue] (B) circle (0.5pt) node[above left]{$2$};
% радиус-вектор
...
\draw[<->] (AA.center) to [bend right=40]node[pos=0.25,above]{$\alpha_{12}$} (AB.center);
\end{tikzpicture}
\caption{Расстояние $d_{12}$ (длина дуги) на поверхности сферы
между точками $1$ и $2$}
\end{figure}

Скалярное произведение радиус-векторов точек $1$ и $2$,
лежащих на поверхности сферы:
\begin{equation*}
(\vect{r}_1,\vect{r}_2) = \norma{\vect{r}_1}\cdot\norma{\vect{r_2}}
\cdot\cos(\alpha_{12}),
\end{equation*}
где $\vect{r}_1$ и $\vect{r}_2$ -- радиус-вектора точек $A$ и $B$,
соответственно:
\begin{gather*}
\vect{r}_1=\mathcal{R}\cdot\left(
\cos(\phi_1)\cos(\lambda_1),
\cos(\phi_1)\sin(\lambda_1),
\sin(\phi_1)
\right)\\
\vect{r}_2=\mathcal{R}\cdot\left(
...
$\alpha_{12}$ -- угол между двумя радиус-векторами:
\begin{equation}
\alpha_{12}=\mathrm{acos}\left(
\dfrac{(\vect{r}_1,\vect{r}_2)}{\norma{\vect{r}_1}\cdot\norma{\vect{r}_2}}
\right),
\end{equation}
$\norma{\vect{r}_1}=\norma{\vect{r}_2}=\mathcal{R}$ --
длины радиус-векторов, с учетом постоянства радиуса Земли.

Расстояние на поверхности сферы (длина дуги большого радиуса)
можно определить из соотношения:
\begin{equation}
d_{12}=\mathcal{R}\cdot\alpha_{12}
\end{equation}
...
%\begin{tikzpicture}[
%>={Straight Barb[angle=45:1.5mm 1]},shorten >=-4pt,
%% объявление функции
%declare function={
%	myfunc(\x,\y) = {
%		sqrt(	(x-0.35)^2	+ (y-1.5)^2		)	+ 
%		sqrt(	(x+0.5)^2	+ (y+2)^2		)	+ 
%		sqrt(	(x-1)^2		+ (y+7)^2		)
%	};
%}
%]
%\begin{axis}[clip=false,ylabel={$y$},zlabel={$f(x,y)$},
%view={-135}{35},%colorbar,
%xtick=\empty,ytick=\empty,ztick=\empty,
%xmin=-6,xmax=6,ymin=-10,ymax=4,
%axis lines=center,axis on top,
%colormap/violet,shader=faceted]
%% путь
%\addplot3[darkred]coordinates{
%(-3.000,-8.000,20.696)
%(0.747,-8.000,16.668)
%(0.747,-2.006,9.776)
%(-0.496,-2.006,8.827)
%(-0.5,-2.0,8.822)
%};
%\addplot3[->,thin,darkred]coordinates{(-0.5,-2.0,8.822) (-0.5,-2.0,0)};
%% целевая функция
%\addplot3[surf,opacity=0.65,very thin,samples=10,
%domain=-6:6,y domain=-10:4]{myfunc(x,y)};
%% начальная точка
%\addplot3[only marks,ball darkred]coordinates {(-3,-8,20.696)}
%node[below]{$(x_0,y_0)$};
%% конечная точка
%\addplot3[only marks,ball darkred]coordinates{(-0.5,-2.0,8.822)};
%\end{axis}
%\end{tikzpicture}
%\end{subfigure}% ВАЖНО!
%
...
%xmin=53,xmax=58,ymin=36,ymax=44,
