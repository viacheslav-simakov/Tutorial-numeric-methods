...
-\lambda\cdot\norma{\nabla{f}(\vect{x}_k)}^2
\end{equation}
где $\lambda>0$ -- малое положительное число;
...
%дана точка $x$, найти точку $\bar{x}$ такую, что $f(\bar{x})<f(x)$. 
%Давайте возьмем точку рядом с $x$, опять же используя линейное приближение 
%$f(\bar{x})\approx f(x)+\nabla f(x)^T(\bar{x}-x)$. 
...
$-\nabla{f}(\vect{x})$:
\begin{equation}
\vect{x}_{k+1} = \vect{x}_k - \vect{\lambda}\cdot\nabla{f}(\vect{x}_k)
\end{equation}
где $k$ -- номер итерационного шага процесса;
$\vect{x}_k$ -- значение неизвестных на $k$-ой итерации;
$\vect{\lambda}=(\lambda_1,\lambda_2,\ldots,\lambda_n)$ --
скорость градиентного спуска в направлении осей координат.
