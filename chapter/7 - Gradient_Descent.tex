...
\node[rectnode,on chain=going left,join,fill=gray!10](a)
{$a=x_1$};
\draw(compare.west) node[above left]{да};
\end{scope}
% нет
\begin{scope}[start branch=b,node distance=15mm]
\node[rectnode,on chain=going right,join,fill=gray!10](b)
{$b=x_2$};
...
\quad
\abs{f(\vect{x}_k)-f(\vect{x}_{k-1})} < \varepsilon_2
\end{equation}
...
\node[rectnode,on chain=going right,join,fill=gray!10](no)
{$\vect{x}_0=\vect{x}_1$};
