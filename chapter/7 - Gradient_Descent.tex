...
Угол между векторами $\vect{a}$ и $\vect{b}$
...
не зависящее от системы координат, то есть \emph{инвариант}:
...
Косинус угла между векторами $\vect{a}$ и $\vect{b}$
\eqref{eq:GD:cos(a,b)} с учетом 
...
%&\cos(\phi_a)\cos(\phi_b)\cdot(
%\cos(\lambda_a)\cdot\cos(\lambda_b)\;+\\
%&\sin(\lambda_a)\cdot\\sin(\lambda_b)\right)\;+\\
%&\sin(\phi_a)\cdot\sin(\phi_b)\;=\\
\end{split}
\end{equation}
или
