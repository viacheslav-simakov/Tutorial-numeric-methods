...
\begin{gather*}
f(\vect{x}_0+\Delta\vect{x})\approx 
f(\vect{x}_0) + \left(\nabla{f(\vect{x}_0)},\Delta\vect{x}\right),
\end{gather*}
где $\Delta\vect{x}=\vect{x}_1-\vect{x}_0$
...
$\nabla_{x}$ и $\nabla_{y}$ -- компоненты вектора градиента 
в декартовой системы координат (на плоскости);
$\lambda_x$ и $\lambda_y$ -- скорость градиентного спуска 
в направлении осей координат, соответственно.
...
не будут выполнены одно и/или несколько неравенств вида:
\begin{equation}
\norma{x^{(k)}-x^{(k-1)}} < \varepsilon_1,
\quad
\norma{f(x^{(k)})-f(x^{(k-1)})} < \varepsilon_2
\end{equation}

% Алгоритм метода градиентного спуска
\emptyline
\subsection{Алгоритм метода градиентного спуска}
\begin{enumerate}
\item 
...
\left\{
\begin{matrix}[rcl]
\diff{\vect{r}}{t}&=&\vect{v}\\[1em]
m\,\diff{\vect{v}}{t}&=&\vect{F} - \alpha\cdot\vect{v}
\end{matrix}
\right.
\end{equation}
где 
$\vect{r}$ -- положение частицы в выбранной системе координат;
$\vect{v}$ и $\alpha$ -- 
...
\end{cases},
\end{equation}
где $\vect{r}_0$ -- начальное приближения для поиска
минимума функции.
