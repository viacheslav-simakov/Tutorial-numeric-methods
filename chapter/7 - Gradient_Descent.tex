...
\begin{tikzpicture}[
declare function={
	myfunc(\x,\y) = {1 - x*exp(-(x-1)^2-(y-2)^2)};
}
]
...
% целевая функция
\addplot3[surf,opacity=0.75,thin,samples=20,
domain=-1:3,y domain=0:4]{myfunc(x,y)};
% начальная точка
\addplot3[only marks,ball darkblue]
coordinates {({-0.5},{3},{myfunc(-0.5,3)})}
node[left]{$(x_0,y_0)$};
\end{axis}
\end{tikzpicture}
\end{subfigure}% ВАЖНО!
...
\foreach \r in {1.5,1,0.5}{
\addplot[fill=gray!25,opacity=0.5,variable=\t,domain=0:360,samples=100]
( {1+\r*cos(t)}, {2+\r*sin(t)} );}
