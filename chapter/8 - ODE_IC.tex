...
о том что неизвестные функции $\vec{u}(t)$ изменяются линейно
в интервале $[t_j,t_{j+1}]$ между двумя соседними узлами 
временной сетки и аппроксимация неизвестных функций 
проводится полиномом первого порядка $\vec L_1(t)$:
\begin{gather*}
\vec u\approx\vec L_1(t)=
\dfrac{t-t_{j+1}}{t_j-t_{j+1}}\cdot\vec u(t_j)+
\dfrac{t-t_j}{t_{j+1}-t_j}\cdot\vec u(t_{j+1}).
\end{gather*}
