...
$\vec f(t, \vec u)$ -- известные функции, зависящие от времени и 
неизвестных функций $\vec u$;
$\vec{\mathring u}$ -- \emph{начальные условия}, 
т.е. значения неизвестных функций в начальный момент времени 
$(t=0)$. 

Запишем в развернутом виде систему 
\eqref{eq:ODE_SYS} линейных обыкновенных дифференциальных 
...
о том что неизвестные функции в интервале между 
узлами временной сетки изменяются линейно. 
Поэтому проводится аппроксимация производных по времени 
от неизвестных функций полиномом первого порядка $\vec L_1(t)$:
\begin{gather}
\diff{\vec u}{t}\approx \vec L^{\prime}_1(t) =
\dfrac{\vec u(t_{j+1})-\vec u(t_j)}{t_{j+1}-t_j},
\end{gather}
где $t_{j+1}$ и $t_j$ -- два последовательных момента времени.

Введем по переменной $t$ сетку, т.е. будем
рассматривать множество моментов времени:
\begin{gather*}
...
\begin{gather*}
y(t_{j+1})=y(t_{j})+\Delta t_{j+1}\cdot f\left(t_j, y(t_{j})\right),\quad y(0)=\mathring y,\quad j=1,2,\dots N
\end{gather*}
...
выражением для решения интегрального уравнения 
