...
\int\limits_{0}^{t_{j+1}}\vect{f}(\xi, \vec{v})\,\mathrm{d}\xi
...
какую-либо норму:
\begin{gather}
\vect{\varepsilon}(t_j)=\vect{v}(t_j) - \vect{u}(t_j)
\end{gather}

Контроль точности приближенного решения 
может вестись покомпонентно или по норме.
Для различных компонент решения задачи $u_i(t_j)$ 
...
\begin{gather*}
\norma{\,\vect{\varepsilon}\,}_{\infty}=\max_{i=1..n} \varepsilon_i
\quad
\norma{\,\vect{\varepsilon}\,}_1=\sum\limits_{i=1}^n \varepsilon_i,
\quad
\norma{\,\vect{\varepsilon}\,}_2=\sqrt{\sum\limits_{i=1}^n \varepsilon_i^2}
\end{gather*}

%
% *** Алгоритм поиска нуля функции одной переменной f(x)=0
...
% начало цикла
\node[startloopnode,on chain,join]{$j=0,1,2,3,\ldots$};
% интервал времени
\node[rectnode,on chain,join]{$\Delta{t}_{j+1}=t_{j+1}-t_j$};
\node[rectnode,on chain,join]
{$\vect{u}(t_{j+1})=\vect{u}(t_j)+
\Delta t_{j+1}\cdot\vect{f}\left(t_j, \vect{u}(t_{j})\right)$};
%\node[srectnode,rectangle split parts=2,on chain,join]
%{
%\nodepart{one}
...
%};
% конец цикла
\node[endloopnode,on chain,join]{$j=m-1$};
% решение
...
\left\{\begin{matrix}
u_1(t_{j+1})&=&u_1(t_{j})+\tau\cdot f_1(t_j),&u_1(0)&=&1\\
u_2(t_{j+1})&=&u_2(t_{j})+\tau\cdot f_2(t_j),&u_2(0)&=&0
\end{matrix}\right.,
\end{gather}
где $\tau=(t_{j+1}-t_j)$ -- временной шаг метода Эйлера, 
...
\left\{\begin{matrix}
\tilde{u}_1(t_{j+1})&=&\tilde{u}_1(t_{j})+\dfrac{f_1(t_{j})+f_1(t_{j+1})}{2}\cdot\tau,&\tilde{u}_1(0)&=&1\\[1em]
\tilde{u}_2(t_{j+1})&=&\tilde{u}_2(t_{j})+\dfrac{f_2(t_{j})+f_2(t_{j+1})}{2}\cdot\tau,&\tilde{u}_2(0)&=&0
\end{matrix}\right.
.\end{gather}
...
\begin{gather*}
\tau=\dfrac{T-t_0}{n} = \dfrac{10-0}{5}=2,
\end{gather*}
...
$\vect{\varepsilon}=|\tilde{\vect{u}}-\vect{u}|$}\\
\midmidrule
$\varepsilon_1$&$0$&$0.6$&$2.2$&$1.6$&$1.4$&$5.2$\\
$\varepsilon_2$&$0$&$0$&$0.6$&$1.8$&$2.4$&$0$\\
\bottomrule
\end{tabular*}
\end{table}
...
составляет $\varepsilon_1=5.2$, 
а для функции $u_2(t)$ -- $\varepsilon_2=2.4$.
...
nodes near coords,
xlabel = {Время, $t$},
ylabel = {Абсолютная погрешность, $\varepsilon$},	% подпись оси y
xmin=1, xmax=11, xtick={0,2,4,6,8,10}, %xticklabels={$0$,$t_1$,,$t_j$,,$t_N$},
