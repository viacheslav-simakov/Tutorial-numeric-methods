...
%
\newpage
\section{Задача Коши для систем обыкновенных дифференциальных уравнений}

Рассмотрим задачу Коши для системы обыкновенных дифференциальных уравнений вида:
\begin{gather}\label{eq:ODE_SYS}
\diff{\vec u}{t} = \vec f(t, \vec u),\quad \vec{u}(0)=\vec{\mathring u}
,\end{gather}
где $\vec u(t)$ -- неизвестные функции, которые подлежат определению;
$\vec f(t, \vec u)$ -- известные функции, зависящие от времени и неизвестных функций $\vec u$;
$\vec{\mathring u}$ -- \textit{начальные условия}, т.е. значения неизвестных функций в начальный момент времени $(t=0)$. 

Запишем в развернутом виде систему \eqref{eq:ODE_SYS} линейных обыкновенных дифференциальных уравнений первого порядка 
и начальные условия:
...
\end{matrix}\right.
,\end{gather}
где $n$ -- количество дифференциальных уравнений в системе.
...
Введем по переменной $t$ равномерную сетку с шагом $\tau>0$, т. е. рассмотрим множество точек:
\begin{gather*}
\omega_{\tau}=\{t_j=j\tau\},\quad j=0,1,2,\dots N
,\end{gather*}
где $j$ -- номер временного интервала;
$\tau$ -- шаг сетки, т.е. временной интервал между двумя последовательными моментами времени;
$N$ -- количество узлов временной сетки.

Основная идея метода Эйлера заключается в предположении, о том что неизвестные функции
в интервале между узлами временной сетки изменяются линейно. 
Поэтому проводится аппроксимация производных по времени от неизвестных функций 
соотношением вида:
\begin{gather}
\diff{\vec u}{t}\approx\dfrac{\vec u(t_{j+1})-\vec u(t_{j})}{\tau}
.\end{gather}

Тогда система дифференциальных уравнений первого порядка \eqref{eq:ODE_SYS} запишется в виде:
\begin{gather}\label{eq:ODE_NUM}
\dfrac{\vec u(t_{j+1})-\vec u(t_{j})}{\tau} = \vec f\left(t_j, \vec u(t_{j})\right)
\end{gather}

Относительно неизвестных $\vec u(t_{j+1})$ это система линейных алгебраических уравнений и 
решение системы \eqref{eq:ODE_NUM} находится явным образом по рекуррентным формулам:
\begin{gather}\label{eq:ODE_EULER}
\vec u(t_{j+1})=\vec u(t_{j})+\tau\cdot\vec f\left(t_j, \vec u(t_{j})\right),
\quad \vec u(0)=\vec{\mathring u},
\quad j=1,2,\dots N
\end{gather}

Запишем полученные рекуррентные соотношения \eqref{eq:ODE_EULER} в развернутом виде:
\begin{gather}\label{eq:ODE_EULER_SYS}
\left\{\begin{matrix}
u_1(t_{j+1})&=&u_1(t_{j})+\tau\cdot f_1\left(t_j, u_1(t_{j}),u_2(t_{j}),\cdots,u_n(t_{j})\right),&u_1(0)&=&\mathring u_1\\%[1em]
u_2(t_{j+1})&=&u_2(t_{j})+\tau\cdot f_2\left(t_j, u_1(t_{j}),u_2(t_{j}),\cdots,u_n(t_{j})\right),&u_2(0)&=&\mathring u_2\\%[1em]
\vdots&=&\ddots&&\vdots&\\[1em]
u_n(t_{j+1})&=&u_n(t_{j})+\tau\cdot f_n\left(t_j, u_1(t_{j}),u_2(t_{j}),\cdots,u_n(t_{j})\right),&u_n(0)&=&\mathring u_n
\end{matrix}\right.
.\end{gather}

Метод Эйлера является простейшим численным методом решения задачи Коши
Недостатки метода являются: малая точность; систематическое накопление ошибок.

Для простоты рассмотрим только одно дифференциальное уравнение с единственным начальным условием:
\begin{gather*}
y(t_{j+1})=y(t_{j})+\tau\cdot f\left(t_j, y(t_{j})\right),\quad y(0)=\mathring y,\quad j=1,2,\dots N
\end{gather*}

На рисунке \ref{fig:EULER} представлена графическая иллюстрация метода Эйлера 
численного решения задачи Коши для одного дифференциального уравнения первого порядка:
% *******************************
%	Ломаная Эйлера
%
\begin{figure}
...
\begin{axis}[
	...
	xmin=-1, xmax=11, xtick={0,2,4,6,8,10}, xticklabels={$t_0$,$t_1$,,$t_j$,,$t_N$},
	ymin=0.5, ymax=7, ytick={1,2,3.5,5,6,6.5}, yticklabels={$y(t_0)$,$y(t_1)$,,$y(t_j)$,,$y(t_N)$},
	xtick style={thick, black},
	ytick style={thick, black},
	grid=major,		
	major grid style={color=black!20, dashed, thin},
]
\addplot[thick, mark=*, mark size=4pt, mark options={fill=lime, draw=black, solid}] coordinates 
{(0,1) (2,2) (4,3.5) (6,5) (8,6) (10,6.5)};
\draw (axis cs:2.2,2) node [right] {$y(t_1)=y(t_0)+\tau f(t_0,y(t_0))$};
\end{axis}
\end{tikzpicture}
\end{center}
\caption{Ломаная Эйлера}\label{fig:EULER}
\end{figure}
% *******************************
...
Интегрирование системы дифференциальных уравнений \eqref{eq:ODE_SYS}
по временной переменной $t$ с учетом начальных условий:
\begin{gather}\label{eq:ODE_SYS_INT}
\hat{\vec u}(t)=\vec u(0)+\int\limits_{0}^{t}\vec f(\xi, \hat{\vec u})d\xi
.\end{gather}

Уравнение \eqref{eq:ODE_SYS_INT} является интегральным уравнением 
для неизвестной функции $\hat{\vec u}(t)$, а его решение 
эквивалентно решению задачи Коши \eqref{eq:ODE_SYS}.

На временной сетке $\omega_\tau$ 
интеграл в правой части равенства \eqref{eq:ODE_SYS_INT}
приближенно вычисляется по формуле трапеций:
\begin{gather}\label{eq:ODE_SYS_INT_TRAP}
\int\limits_{0}^{t_{j+1}}\vec f(\xi, \hat{\vec u})d\xi\approx\sum\limits_{k=0}^{j}
\dfrac{
\vec f\left(t_{k},\hat{\vec u}(t_{k})\right)+
\vec f\left(t_{k+1},\hat{\vec u}(t_{k+1})\right)
}{2}\cdot\tau
.\end{gather}

Воспользовавшись \eqref{eq:ODE_SYS_INT_TRAP}, 
выражением для решения интегрального уравнения \eqref{eq:ODE_SYS_INT} 
можно записать в рекуррентной форме:
\begin{gather}
\hat{\vec u}(t_{j+1})=\hat{\vec u}(t_{j})+
\dfrac{
\vec f\left(t_{j},\hat{\vec u}(t_{j})\right)+
\vec f\left(t_{j+1},\hat{\vec u}(t_{j+1})\right)
}{2}\cdot\tau
.\end{gather}

Для определения приближенного значения решения интегрального уравнения 
могут быть использованы значения неизвестных функций 
${\vec u}(t_{j})$, 
рассчитанные по методу Эйлера на $j$-ом временном слое:
\begin{gather}
\hat{\vec u}(t_{j+1})\approx\hat{\vec u}(t_{j})+
\dfrac{
...
В качестве предельной абсолютной погрешности приближенного решения ${\vec u}(t_{j})$ 
задачи Коши \eqref{eq:ODE_SYS} можно принять величину:
\begin{gather}
\epsilon_i(t_j)=|\hat{u}_i(t_j) - {u}_i(t_j)|,\quad i=1,2,\dots n
\end{gather}
