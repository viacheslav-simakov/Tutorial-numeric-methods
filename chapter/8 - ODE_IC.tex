...
\node[signalnode,on chain,join](forloop){$j=0,1,\ldots,(m-1)$};
% интервал времени
...
% конец цикла
\node[rectnode,on chain,join](jloop){$j=j+1$};
\draw[<-,thick](forloop.west) -- ([xshift=-30mm]forloop.west) |- (jloop.west);
% решение
\node[IOnode,on chain](output)
{$\vect{u}(\vect{t})=\{u_1(\vect{t}),u_2(\vect{t}),\ldots,u_n(\vect{t})\}$};
\draw[->,thick](forloop.east) -- ([xshift=+30mm]forloop.east) |- (output.east);
% конец
