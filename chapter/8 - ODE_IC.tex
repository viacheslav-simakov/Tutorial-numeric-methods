...
\begin{scheme}
% начальное условие
\node[IOnode,on chain,join]{$\{t_j\}, \{u_i(0)\}, \{f_i(t,\{u_i(t)\})\}$};
% начало цикла
...
\node[rectnode,on chain,join](i-loop){$i=0$};
% правая часть системы уравнений
\node[rectnode,on chain,join](euler)
{$F_i=f_i\left(t_j, u_0(t_{j}), u_1(t_{j}), \ldots, u_n(t_{j})\right)$};
% приращение функции
\node[rectnode,on chain,join]
{$u_i(t_{j+1})=u_i(t_j)+\Delta t_{j+1}\cdot F_i$};
% конец цикла
\node[ifthenelsenode,on chain,join](i-end){$i<n$};
\draw(i-end.west) node[above left]{да};
\draw(i-end.south) node[below left]{нет};
\begin{scope}[start branch=i,node distance=25mm]
\node[rectnode,on chain=going left,join](i++){$i=i+1$};
\end{scope}
% условие
...
\end{scope}
% решение
\node[IOnode,on chain,join](output){$\{u_i\left(\{t_j\}\right)\}$};
% связи
\draw[->,thick](i++.north) |- (euler.west);
\end{scheme}
...
\begin{scheme}
% начальное условие
\node[IOnode,on chain,join]{$\{t_j\}, \{u_i\left(\{t_j\}\right)\}$};
...
\node[rectnode,on chain,join]{$i=0$};
% F
\node[rectnode,on chain,join](i-loop){$
F_j=f_i\left(t_j, \{u_i(t_{j})\}\right),\quad
F_{j+1}=f_i\left(t_{j+1}, \{u_i(t_{j+1})\}\right)$};
% F1
%\node[rectnode,on chain,join,text width=10cm]{
%$F_j=f_i\left(t_j, u_0(t_{j}), u_1(t_{j}), \ldots, u_n(t_{j})\right)$\\
%$F_{j+1}=f_i\left(t_{j+1}, u_0(t_{j+1}), u_1(t_{j+1}), \ldots, u_n(t_{j+1})\right)$
%};
%% F2
%\node[rectnode,on chain,join]
%{$F_{j+1}=f_i\left(t_{j+1}, u_0(t_{j+1}), u_1(t_{j+1}), \ldots, u_n(t_{j+1})\right)$};
...
\node[rectnode,on chain,join]
{$v_i(t_{j+1})=v_i(t_{j})+
\dfrac{F_{j+1} + F_{j}}{2}\cdot\Delta t_{j+1}$};
% погрешность
\node[rectnode,on chain,join]{$\epsilon_i(t_j)=v_i(t_j) - u_i(t_j)$};
% конец i-цикла
\node[ifthenelsenode,on chain,join](i-end){$i<n$};
\draw(i-end.west) node[above left]{да};
\draw(i-end.south) node[below right]{нет};
\begin{scope}[start branch=i,node distance=40mm]
\node[rectnode,on chain=going left,join](i++){$i=i+1$};
\end{scope}
% конец j-цикла
\node[ifthenelsenode,on chain,join](compare){$j<m$};
...
\node[rectnode,on chain,join](j++){$j=j+1$};
\end{scope}
% решение
\node[IOnode,on chain,join]
{$\{\epsilon_i\left(\{t_j\}\right)\}$};
% связи
\draw[->,thick] (i++.north) |- (i-loop.west);
\draw[->,thick] (j++.north) |- (loop.east);
\end{scheme}
...
\begin{enumerate}
\item
Определим значения неизвестных функций $v_1$ и $v_2$ в точке $t_1=2$:
\begin{gather*}
\left\{\begin{array}{rcl}
v_1(2)&=&v_1(0)+\tau\cdot\dfrac{f_1(0)+f_1(2)}{2}
=1+2\cdot\dfrac{0+(-0.6)}{2}=0.4\\[1em]
v_2(2)&=&v_2(0)+\tau\cdot\dfrac{f_2(0)+f_2(2)}{2}
=0+2\cdot\dfrac{-0.5+(-0.5)}{2}=-1
\end{array}\right..
\end{gather*}
\item
Определим значения неизвестных функций $v_1$ и $v_2$ в точке $t_2=4$:
\begin{gather*}
\left\{\begin{array}{rcl}
v_1(4)&=&v_1(2)+\tau\cdot\dfrac{f_1(2)+f_1(4)}{2}
=0.4+2\cdot\dfrac{(-0.6)+(-1.2)}{2}=-2.4\\[1em]
v_2(4)&=&v_2(2)+\tau\cdot\dfrac{f_2(2)+f_2(4)}{2}
=-1+2\cdot\dfrac{-0.5+0.1}{2}=-1.4
\end{array}\right..
\end{gather*}
\item
Определим значения неизвестных функций $v_1$ и $v_2$ в точке $t_3=6$:
\begin{gather*}
\left\{\begin{array}{rcl}
v_1(6)&=&v_1(4)+\tau\cdot\dfrac{f_1(4)+f_1(6)}{2}
=-2.4+2\cdot\dfrac{-1.2+(-0.6)}{2}=-4.2\\[1em]
v_2(6)&=&v_2(4)+\tau\cdot\dfrac{f_2(4)+f_2(6)}{2}
=-1.4+2\cdot\dfrac{0.1+1.3}{2}=0
\end{array}\right..
\end{gather*}
\item
Определим значения неизвестных функций $v_1$ и $v_2$ в точке $t_4=8$:
\begin{gather*}
\left\{\begin{array}{rcl}
v_1(8)&=&v_1(6)+\tau\cdot\dfrac{f_1(6)+f_1(8)}{2}
=-4.2+2\cdot\dfrac{-0.6+2.4}{2}=-2.4\\[1em]
v_2(8)&=&v_2(6)+\tau\cdot\dfrac{f_2(6)+f_2(8)}{2}
=0+2\cdot\dfrac{1.3+1.9}{2}=3.2
\end{array}\right.
.\end{gather*}
\item
Определим значения неизвестных функций $v_1$ и $v_2$ в точке $t_5=10$:
\begin{gather*}\begin{array}{rcl}
...
\begin{gather*}
\left\{\begin{array}{rcl}
v_1(10)&=&v_1(8)+\tau\cdot\dfrac{f_1(8)+f_1(10)}{2}
=-2.4+2\cdot\dfrac{2.4+6.2}{2}=6.2\\[1em]
v_2(10)&=&v_2(8)+\tau\cdot\dfrac{f_2(8)+f_2(10)}{2}
...
\begin{axis}[
xlabel = {$t$},	% подпись оси x
ylabel = {$u_1, v_1$},	% подпись оси y
xmin=-1, xmax=11, xtick={0,2,4,6,8,10},
ymin=-6, ymax=7, ytick={-6,-4,-2,0,2,4,6},
legend pos={north west}]
\addplot[ball darkgreen]coordinates{(0,1) (2,1) (4,-0.2) (6,-2.6) (8,-3.8) (10,1)};
\addlegendentry{$u_1$};
...
\begin{axis}[
xlabel = {$t$},	% подпись оси x
ylabel = {$u_2, v_2$},	% подпись оси y
xmin=-1, xmax=11, xtick={0,2,4,6,8,10},
ymin=-3, ymax=5, ytick={-2,0,2,4},
legend pos={north west}]
\addplot[ball darkred]coordinates{(0,0) (2,-1) (4,-2) (6,-1.8) (8,0.8) (10,4.6)};
\addlegendentry{$u_2$};
\addplot[thick, mark=*, mark size=3pt, mark options={fill=white, draw=black, solid}]
...
\midmidrule
$v_1(t_i)$&$1$&$0.4$&$-2.4$&$-4.2$&$-2.4$&$6.2$\\
$v_2(t_i)$&$0$&$-1$&$-1.4$&$0$&$3.2$&$4.6$\\
\midrule
\multicolumn{7}{l}{Абсолютная погрешность 
$\vect{\epsilon}=\abs{\vect{v}-\vect{u}}$}\\
\midmidrule
