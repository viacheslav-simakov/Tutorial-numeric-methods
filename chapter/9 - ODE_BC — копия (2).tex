...
%
\begin{table}[H]
\caption{Основные виды краевых условий для системы обыкновенных
дифференциальных уравнений \eqref{eq:ODE(BC):1D}}
...
node[draw](D) {$u(0)=u_0\equiv\dfrac{c_0}{b_0}$};
\node[below] at (D.south) {Задача Дирихле};
% Задача Неймана
\draw[shape=rectangle, minimum height=3em, minimum width=5em]
($(G.south east)+(3em,-5em)$)
node[draw](N) {$w(0)=w_0\equiv\dfrac{c_0}{a_0}$};
\node[below] at (N.south) {Задача Неймана};
% стрелки
\draw[-latex,thick] (G.west) -| (D.north) node[left,midway] {$a_0\equiv0$};
\draw[-latex,thick] (G.east) -| (N.north) node[right,midway] {$b_0\equiv0$};
\end{tikzpicture}
% *******************************************
...
% входные данные
\node[IOnode,on chain,join]{$\ell, n$};
% начало цикла
\node[rectnode,on chain,join]{$i=0$};
% x[i]
\node[rectnode,on chain,join](loop)
{$x_i=\dfrac{i}{n}\cdot\ell$};
% конец цикла
\node[ifthenelsenode,on chain,join](if){$i<n$};
\draw(if.east) node[above right]{да};
\draw(if.south) node[below right]{нет};
% инкремент
\begin{scope}[start branch=b,node distance=15mm]
\node[rectnode,on chain,join](increment){$i=i+1$};
\draw[->,thick] (increment.north) |- (loop.east);
\end{scope}
% выходные данные
\begin{scope}[node distance=15mm]
\node[IOnode,on chain,join]{$\{x_i\}$};
\end{scope}
% стоп
...
&\approx r(x_{i-1})\cdot(x_i-x_{i-1/2})+r(x_i)\cdot(x_{i+1/2}-x_i)=\\[1em]
&=r_{i-1}\cdot\dfrac{x_i-x_{i-1}}{2}+r_i\cdot\dfrac{x_{i+1}-x_i}{2}.
\end{split}
\end{equation}
...
&\approx q_{i-1}\cdot\dfrac{x_i-x_{i-1}}{2}+q_i\cdot\dfrac{x_{i+1}-x_i}{2}.
\end{split}
\end{gather}
...
\approx r(x_{n-1})\cdot(x_n-x_{n-1/2})=
r_{n-1}\cdot\dfrac{x_n-x_{n-1}}{2},
\end{gather*}
\begin{gather*}%\label{eq:ODE+BC:RIGHT:heat source}
\varepsilon_n=\intf{x_{n-1/2}}{x_n}{ q(x) }
\approx q(x_{n-1})\cdot(x_n-x_{n-1/2})=
q_{n-1}\cdot\dfrac{x_n-x_{n-1}}{2}.
\end{gather*}
...
$};
% i = 1
\node[rectnode,on chain,join]{$i=1$};
...
\node[rectnode,on chain,join]
{$\varepsilon_i=
q_{i-1}\cdot\dfrac{x_i-x_{i-1}}{2}+q_i\cdot\dfrac{x_{i+1}-x_i}{2}
$};
% конец цикла
\node[ifthenelsenode,on chain,join](if){$i<(n-1)$};
\draw(if.east) node[above right]{да};
\draw(if.south) node[below right]{нет};
% инкремент
\begin{scope}[start branch=b,node distance=21mm]
\node[rectnode,on chain=going right,join](increment){$i=i+1$};
\draw[->,thick] (increment.north) |- (loop.east);
\end{scope}
% lambda[n], sigma[n] varepsoln[n]
\node[rectnode,on chain,join]{$
\sigma_n=r_{n-1}\cdot\dfrac{x_n-x_{n-1}}{2},
\quad
\varepsilon_n=q_{n-1}\cdot\dfrac{x_n-x_{n-1}}{2}$};
% выходные данные
\node[IOnode,on chain,join]
...
% b[0], c[0], y[0]
\node[rectnode,on chain,join]{$
b_0=\lambda_0,\quad c_0=b_0+\sigma_0+p_0
$};
%
\node[rectnode,on chain,join]{$y_0=\varepsilon_0+s_0$};
% начало цикла
\node[rectnode,on chain,join]{$i=1$};
% lambda[i] - теплоперенос
\node[rectnode,on chain,join](loop)
...
$};
%
\node[rectnode,on chain,join]{$y_i=\varepsilon_i$};
% конец цикла
\node[ifthenelsenode,on chain,join](if){$i<(n-1)$};
\draw(if.east) node[above right]{да};
\draw(if.south) node[below right]{нет};
% инкремент
\begin{scope}[start branch=b,node distance=28mm]
\node[rectnode,on chain=going right,join](increment){$i=i+1$};
\draw[->,thick] (increment.north) |- (loop.east);
\end{scope}
% lambda[n], sigma[n] varepsoln[n]
...
c_n=a_n+\sigma_n+p_{\ell}$};
%
\node[rectnode,on chain,join]{$y_n=\varepsilon_n+s_{\ell}$};
% выходные данные
\node[IOnode,on chain,join]
{$\{a_i\},\{b_i\},\{c_i\},\{y_i\}$};
% стоп
...
% *************************************************************
\emptyline
\subsubsection{Метод прогонки для
решения систем линейных уравнений с матрицами специального вида}
...
\node[beginendnode,on chain,join]{начало};
% входные данные
\node[IOnode,on chain,join]{$\{a_i\},\{b_i\},\{c_i\},\{y_i\}$};
% alpha[1], beta[1]
\node[rectnode,on chain,join]{$
...
\end{scope}
% выходные данные
\node[IOnode,on chain,join]{$\{\alpha_i\}, \{\beta_i\}$};
% стоп
\node[beginendnode,on chain,join]{конец};
\end{tikzpicture}
\caption{Блок-схема алгоритма вычислений\linebreak 
...
$};
% начало цикла
\node[rectnode,on chain,join]{$i=(n-1)$};
% u[i]
\node[rectnode,on chain,join](loop){$
u_i=\alpha_{i+1}\cdot u_{i+1} + \beta_{i+1}$};
% конец цикла
\node[ifthenelsenode,on chain,join](if){$i>0$};
\draw(if.east) node[above right]{да};
\draw(if.south) node[below right]{нет};
% инкремент
\begin{scope}[start branch=b,node distance=16mm]
\node[rectnode,on chain=going right,join](increment){$i=i-1$};
\draw[->,thick] (increment.north) |- (loop.east);
\end{scope}
% выходные данные
...
w_n=w_{n-1/2}-\sigma_n\cdot u_n + \varepsilon_n$};
% выходные данные
\node[IOnode,on chain,join]{$\{w_0,w_{i+1/2},w_n\}$};
% стоп
\node[beginendnode,on chain,join]{конец};
\end{tikzpicture}
