...
\node[rectnode,on chain,join]{$x_{\min}=\dfrac{a+b}{2}$};
\end{scope}
...
\begin{scope}[node distance=12mm]
\node[rectnode,on chain,join](min){$\vect{x}_{\min}=\vect{x}$};
\end{scope}
% выход f
\draw[->](compare-f.west) -- ([xshift=-15mm]compare-f.west) |- (min.west);
\node[rrectnode,on chain,join](stop){стоп};
\end{tikzpicture}
% *******************************************
\caption{Блок-схема алгоритма нахождения минимума\linebreak 
...
\draw (compare.south) node[below left]{да};
\begin{scope}[node distance=12mm]
\node[rectnode,on chain,join]{$\vect{x}_{\min}=\vect{x}_1$};
\end{scope}
\node[rrectnode,on chain,join]{стоп};
\end{tikzpicture}
% *******************************************
