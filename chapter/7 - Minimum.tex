...
Метод применим к не дифференцируемым функциям и всегда 
сходится. 
...
% f(x1) < f(x2)
\node[ifthenelsenode,on chain,join](compare){$f(x_1)>f(x_2)$};
\draw(compare.west) node[above left]{да};
\draw(compare.east) node[above right]{нет};
% да
\begin{scope}[start branch=a]
\node[rectnode,on chain=going left,join](a){$a=x_1$};
\end{scope}
% нет
\begin{scope}[start branch=b]
\node[rectnode,on chain=going right,join](b){$b=x_2$};
\end{scope}
% ошибка
\node[rectnode,on chain](error){$\Delta=b-a$};
%\begin{scope}[start branch=p]
%\node[pointnode,on chain](error){};%{$\Delta=b-a$};
%\end{scope}
% ошибка
\node[ifthenelsenode,on chain,join](exit)
{$\Delta<\epsilon$};
\draw (exit.east) node[above right]{нет};
\draw (exit.south) node[below right]{да};
% связи
\draw[->,thick] (a.south) |- (error.west);
\draw[->,thick] (b.south) |- (error.east);
\draw[->,thick] (exit.east) -- ([xshift=5cm]exit.east) |- (x1x2.east);
% результат
\node[IOnode,on chain,join]{$x_{\min}=\dfrac{a+b}{2}$};
\end{scheme}
\caption{Блок-схема алгоритма нахождения минимума\linebreak 
функции одного переменного
$x_{\min}=\arg\left(\min\limits_{a\leqslant x \leqslant b} f(x)\right)$ }
...
и определяют область поиска минимума функции:
\begin{equation*}
...
\begin{equation*}
f_1(x_1)=f(x_1,x_{20}=\const,\ldots,x_{n0}=\const)
\end{equation*}
\item
Используя метод одномерной минимизации, находится 
минимум функции одной переменной $f_1(x_1)\to\min$, 
который можно обозначить через $m_1$.
\begin{equation*}
m_1=\arg\left(\min_{a_1 \leqslant x_1 \leqslant b_1}f_1(x_1)\right)
\end{equation*}
\item
...
то есть находится минимум функции $f_2(x_2)$:
\begin{equation*}
f_2(x_2)=f(m_1,x_2,x_{30}=\const,\ldots,x_{n0}=\const),
\end{equation*}
который обозначим $m_2$:
\begin{equation*}
m_2=\arg\left(\min_{a_2 \leqslant x_2 \leqslant b_2}f_2(x_2)\right)
\end{equation*}
\item
...
\end{equation*}
\item
Процесс спуска по координатам повторяется для всех 
остальных переменных задачи $x_3,x_4,\ldots,x_n$, 
а переход в точку $\vect{x}_m=(m_1,m_2,\ldots,m_n)$
...
(рисунок \ref{fig:min:scheme coordinates}).
\end{enumerate}
...
%
% Алгоритм поиска минимума функции многих переменных f(x1,x2,...,xn)
% методом по координатного спуска
%
\begin{figure}[H]\centering
\begin{scheme}
% входные данные
\node[IOnode,on chain,join]
{$\vect{x}_0, \mathcal{D}, \epsilon_1$};
% начальное приближение
\node[rectnode,on chain,join](x){$\vect{x}=\vect{x}_0$};
% начало цикла
%\node[startloopnode,on chain,join](loop){$i=1,2,3,\ldots,n$};
\node[rectnode,on chain,join](loop){$i=1$};
% min
%\node[srectnode,rectangle split parts=2,on chain,join](min)
%{
%\nodepart{one}
%$x_j=\const, \quad\forall j \ne i, \quad j=1,2,\ldots,n$
%\nodepart{two}
%$x_i=\arg\left(\min\limits_{a_i \leqslant x_i \leqslant b_i}f(x_i)\right)$
%};
\node[subnode,on chain,join](sub){\nodepart{two}
$x_i=\arg\left(\min\limits_{a_i \leqslant x_i \leqslant b_i}f(x_i)\right)$};
% смещение
%\node[rectnode,on chain,join]
%{$\vect{x}=(x_1,\ldots,m_i,\ldots,x_n)$};
% конец цикла
\node[ifthenelsenode,on chain,join](endloop){$i>n$};
\draw(endloop.west) node[above left]{нет};
\draw(endloop.south) node[below right]{да};
% инкремент
\begin{scope}[start branch=loop]
\node[rectnode,on chain=going left,join](increment){$i=i+1$};
\end{scope}
% f(x)<f(x0)
%\node[ifthenelsenode,on chain,join](compare-f)
%{$f(\vect{x})<f(\vect{x}_0)$};
%\draw(compare-f.south) node[below right]{да};
%\draw(compare-f.west) node[above left]{нет};
% ошибка
\node[ifthenelsenode,on chain,join](compare)
{$\norma{\vect{x}-\vect{x}_0}\leqslant\epsilon_1$};
\draw (compare.south) node[below right]{да};
\draw(compare.east) node[above right]{нет};
% нет
\begin{scope}[start branch=b]
\node[rectnode,on chain=going right,join](next)
{$\vect{x}_0=\vect{x}$};
\end{scope}
% минимум
\node[IOnode,on chain,join](min){$\vect{x}_{\min}=\vect{x}$};
% связи
\draw[->,thick](increment.north) |- (sub.west);
\draw[->,thick](next.north) |- (loop.east);
\end{scheme}
\caption{Блок-схема алгоритма нахождения минимума\linebreak 
функции $f(\vect{x})$ многих переменных методом 
спуска по координатам}
\label{fig:min:scheme coordinates}
\end{figure}
...
$\nabla$ \emph{не коммутативны} и не антикоммутативные, 
...
% выходные данные
\node[IOnode,on chain,join]{$\vect{x}_{\min}=\vect{x}$};
\end{scheme}
\caption{Блок-схема алгоритма нахождения минимума\linebreak 
...
\subsubsection{Метод спуска по координатам}
...
node[sloped,pos=0.5,below,rotate=180]{$\vect{b}$};
% угол
...
\begin{equation*}
f_1(\phi)=f(\phi, \lambda_0)=f(\phi, \lambdaStart)
\end{equation*}

Находим минимум вспомогательной функции 
одной переменной $f_1(\phi)$, используя метод 
одномерной минимизации:
\begin{equation*}
\phi_{\min}
=\arg\left(
\min_{\phi_{\inf}\leqslant\phi\leqslant\phi_{\sup}}f_1(\phi)
\right)
=\phiMin
\end{equation*}
...
\begin{equation*}
f_2(\lambda)=f(\phi_{\min},\lambda)=f(\phiMin,\lambda)
\end{equation*}

Находим минимум функции одной переменной $f_2(\lambda)$,
используя метод одномерной минимизации:
\begin{equation*}
\lambda_{\min}
=\arg\left(
\min_{\lambda_{\inf}\leqslant\lambda\leqslant\lambda_{\sup}}f_2(\lambda)
\right)
=\lambdaMin
\end{equation*}
...
%% смещение по lambda
\addplot[->,gray,thin,dashed,opacity=0.5,shorten >=3pt]
...
%
\begin{figure}[H]\centering
\begin{tikzpicture}[>={Straight Barb[angle=45:1.5mm 1]}]
\begin{axis}[enlargelimits=0.2,grid=none,
xlabel={широта $\phi$},ylabel={долгота $\lambda$},
...
%%
\draw[darkred,->,shorten >= 3pt] (axis cs:52.0,44.0) -- (axis cs:54.66658,39.49103);
\draw[darkred,->,shorten >= 3pt] (axis cs:54.66658,39.49103) -- (axis cs:\phiMin,\lambdaMin);
%% конечное положение
\addplot[ball darkred]coordinates{(54.66658,39.49103)};
...
\item
Полученные данные в процессе выполнения
минимизации целевой функции методом 
по координатного спуска представлены в таблице 
\ref{tab:r(iteration)}.
%
% Таблица: траектория поиска минимума
%
\begin{table}[H]
\caption{Траектория поиска местоположения склада торговой сети}
\label{tab:r(iteration)}
\small
\begin{tabular*}{\textwidth}{%
@{\extracolsep{\fill}}p{2cm} p{2.5cm} p{2.5cm} p{2cm} p{5cm}}
\toprule
Итерация&\multicolumn{2}{l}{Местоположение склада}%
&Шаг $\xi$&Расстояние $f(\phi,\lambda)$, км\\
\cmidrule{2-3}
&широта $\phi$&долгота $\lambda$\\
\midmidrule
$0$&$52.00000$&$44.00000$&$0.00000$&$1338.696$\\
$1$&$54.66658$&$39.49103$&$5.23846$&$944.004$\\
$2$&$55.94734$&$38.88818$&$1.41555$&$845.572$\\
$3$&$55.88973$&$38.83941$&$0.07548$&$845.314$\\
\bottomrule
\end{tabular*}
\end{table}

На рисунке \ref{fig:f(iteration)} представлена зависимость
суммарного расстояния между складом и всеми магазинами 
от номера итерации $k$ поиска минимума целевой функции:
%
% График: уменьшение функции в итерациях
%
\begin{figure}[H]\centering
\begin{tikzpicture}
\begin{axis}[enlargelimits=0.2,
xtick={0,1,2,3},ytick={800,1000,1200,1400},
xlabel={Номер итерации $k$},
ylabel={$f(\phi,\lambda)$, км},ylabel style={yshift=+1ex},
y tick label style={/pgf/number format/.cd,set thousands separator={}},
]
\addplot[darkred,mark=*,mark size=3pt,mark options={fill=white}]coordinates{(0,1338.696)(1,944.004)(2,845.572)(3,845.314)};
\end{axis}
\end{tikzpicture}
\caption{Зависимость суммарного расстояния $f(\phi,\lambda)$
между складом и магазинами торговой сети от числа итераций}
\label{fig:f(iteration)}
\end{figure}

На рисунке представлена зависимость изменение местоположения 
склада $\xi$ от номера итерации $k$ процесса поиска минимума
целевой функции:
%
% График: уменьшение функции ошибки
%
\begin{figure}[H]\centering
\begin{tikzpicture}
\begin{axis}[%axis on top,
xtick={0,1,2,3},ytick={0.1,1,10},
enlargelimits=0.2,
xlabel={Номер итерации $k$},
ylabel={Шаг итерации $\xi$},ymode=log,
%y tick label style={/pgf/number format/.cd,set thousands separator={}},
]
\addplot[darkred,mark=*,mark size=3pt,mark options={fill=white}]coordinates{(1,5.23846)(2,1.41555)(3,0.07548)};
\end{axis}
\end{tikzpicture}
\caption{Изменение местоположения склада $\xi$ 
на $k$-ом шаге итерационного процесса}
\label{}
\end{figure}
