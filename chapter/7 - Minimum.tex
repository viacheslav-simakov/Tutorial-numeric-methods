...
% связи
\draw[->,thick] (no.north) |- (loop.east);
\draw[->,thick](compare-f.west) -- ([xshift=-15mm]compare-f.west) |- (min.west);
\end{scheme}
\caption{Блок-схема алгоритма нахождения минимума\linebreak 
...
% сравнение
\node[ifthenelsenode,on chain,join](compare)
{$\norma{\vect{x}_1-\vect{x}_0}\leqslant\epsilon_1$};
\draw(compare.east) node[above right]{нет};
\draw (compare.south) node[below right]{да};
% нет
\begin{scope}[start branch=b]
...
% выходные данные
\node[IOnode,on chain,join]{$\vect{x}_{\min}=\vect{x}_1$};
\end{scheme}
\caption{Блок-схема алгоритма нахождения минимума\linebreak 
функции $f(\vect{x})$ многих переменных 
