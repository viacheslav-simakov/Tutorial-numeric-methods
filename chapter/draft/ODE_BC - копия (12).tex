...
\input{ODE_BC_fig/ODE_BC_fig(Dirichlet_Neumann).tex}
...
($q<0$ -- источников тепла) вдоль боковой поверхности стержня;
$r(x)$ -- коэффициент конвекционного теплообмена боковой поверхности
стержня с окружающей средой;
$g(x)>0$ -- мощность распределенных источников ($q(x)<0$ стоков)
тепла внутри стержня.

Граничные условия \eqref{eq:ODE_BC2} определяют процесс теплообмена 
...
тела прямо пропорционален разнице 
температур между телом и окружающей средой``:
\begin{gather*}
Q = \mathring{\kappa}\cdot\brackets{\mathring{u} - u}
,\end{gather*}
где $\mathring{\kappa}$ --  коэффициент конвективной теплопередачи;
$\mathring{u}$ -- тепература окружающей среды;
$u$ -- тепература тела.

Процесс выделения или поглощения тепла в твердом теле могут быть связаны с рядом процессов:
\begin{enumerate}
\item
...
%	Сетка
%
\subsubsection{Рассчетная сетка}
...
\begin{tikzpicture}[framed]
% контрольная область
\draw[shape=rectangle, minimum height=3cm, minimum width=5cm] 
(0,0) node[thick,color=black,draw,color=black,fill=orange!10,] (A)
{$\int\limits_{x_{i-\onehalf}}^{x_{i+\onehalf}}g(x)dx$};
% слагаемые уравнения баланса
\draw[-Latex,line width=2pt,blue] (A.east) -- +(3,0) node[above,midway] {$w(x_{i+\onehalf})$};
\draw[Latex-,line width=2pt,blue] (A.west) -- +(-3,0) node[above,midway] {$w(x_{i-\onehalf})$};
\draw[-Latex,line width=2pt,red] (A.north) -- +(0,3) node[right,midway] {$\int\limits_{x_{i-\onehalf}}^{x_{i+\onehalf}}r(x)\cdot u(x)dx$};
% ось OX
\draw[-Latex,very thick,black] ($(A.south west)+(-4,-1em)$) -- ($(A.south east) + (4,-1em)$) node[below,yshift=-1ex,xshift=-1em] {$x$};
...
\draw[very thick,black] ($(A.south)+(0,-0.75em)$) -- ($(A.south) + (0,-1.25em)$) node[below] {$x_{i}$};
\end{tikzpicture}
\end{center}
\caption{Иллюстрация баланса тепла на участке стержня $[x_{i-\onehalf},x_{i+\onehalf}]$}
\end{figure}
...
\int\limits_{x_{i}}^{x_{i\pm1}}u^{\prime}(x)dx
=-\int\limits_{x_{i}}^{x_{i\pm1}}\dfrac{w(x)}{k(x)}dx,\\[1em]
\label{eq:ODE_BC_FourierLaw}
u(x_{i\pm1})-u({x_{i}})=-\int\limits_{x_{i}}^{x_{i\pm1}}\dfrac{w(x)}{k(x)}dx
.\end{gather}
...
в выражениях \eqref{eq:ODE_BC_CON} и \eqref{eq:ODE_BC_FourierLaw}
необходимо вычислять определенные интегралы на
отрезках $[x_{i-\onehalf},x_{i+\onehalf}]$.
Во многих практически важных приложениях 
...
например, симметричной формулой прямоугольников 
(рисунок \ref{eq:ODE_BC_INT}):
\begin{gather*}
\int\limits_{x_{i-1}}^{x_i}y(x)dx\approx y(x_{i-\onehalf})\cdot (x_{i}-x_{i-1})=y_i\cdot h_i
,\end{gather*}
где $y_{i-\onehalf}=y(x_{i-\onehalf})$ -- значение функции в промежуточном узле рассчетной сетки;
$h_i=(x_{i}-x_{i-1})$ -- шаг сетки.
...
%(рисунок \ref{fig:ODE_mesh}),
Приближенное значение интеграла в \eqref{eq:ODE_BC_FourierLaw}
выразим воспользовавшись симметричной формулой прямоугольников и
найдем поток тепла в промежуточных узлах сетки $w(x_{i\pm\onehalf})$:
\begin{gather}
\notag
u(x_{i\pm1})-u(x_{i})\approx-\dfrac{w(x_{i\pm\onehalf})}{k_{i\pm\onehalf}}\cdot(x_{i\pm1}-x_{i}),\\[1em]
\label{eq:ODE_BC_w}
w(x_{i\pm\onehalf})\approx-{k}_{i\pm\onehalf}\cdot\dfrac{u(x_{i\pm1})-u(x_{i})}{x_{i\pm1}-x_{i}}
\end{gather}
где $k_{i\pm\onehalf}=k(x_{i\pm\onehalf})$ -- значение коэффициента 
температуропроводности в промежуточных узлах сетки $\{x_{i\pm\onehalf}\}$.

% коэффициенты r,g
Интегральный (суммарный) темп поглощения и выделения тепла 
в выражении \eqref{eq:ODE_BC_CON} определим приближенно, 
воспользовавшись формулой прямоугольников:
\begin{gather}\label{eq:ODE_BC_rg}
\begin{array}{rcl}
% <r>
\int\limits_{x_{i-\onehalf}}^{x_{i+\onehalf}}r(x)\cdot u(x)dx
\approx\brackets{ r(x_i)\cdot u(x_i) }\cdot \overline{h}_{i}&=&\overline{r}_i\cdot u(x_i)\\
% <g>
\int\limits_{x_{i-\onehalf}}^{x_{i+\onehalf}}g(x)dx
\approx g(x_i)\cdot\overline{h}_{i}&=&\overline{g}_i
\end{array}
,\end{gather}
где 
$\overline{r}_i=r_i\cdot \overline{h}_{i}$ и 
$\overline{g}=g_i\cdot \overline{h}_{i}$ -- 
интегральный темп генерации и скорость поглощения тепла 
в пределах контрольного отрезка $[x_{i-\onehalf},x_{i+\onehalf}]$;
$r_i=r(x_i)$ и $g_i=g(x_i)$ -- значение коэффициентов в узлах сетки $\{x_i\}$.
