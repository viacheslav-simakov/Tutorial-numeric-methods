...
\int\limits_{x_{i}}^{x_{i+1}}y(x)dx\approx y(x_{i})\cdot (x_{i+1}-x_{i})=y_{i}\cdot h_{i+1}
,\end{gather}
...
конвекционного теплового потока через всю 
боковую поверхность стержня:
\begin{gather*}
\intf{ x_{i-\onehalf} }{ x_{i+\onehalf} }{ r(x)\cdot u(x) }\approx u(x_i)\cdot\overline{r}_i
,\end{gather*}
где $\overline{r}_i$ -- интегральный коэффициент конвекционного теплообмена:
\begin{gather*}
\overline{r}_i=\intf{ x_{i-\onehalf} }{ x_{i+\onehalf} }{ r(x) }
.\end{gather*}
...
% <r>
\overline{r}_i=\bracketssquare{~ 
\intf{ x_{i-\onehalf} }{ x_{i} }{ r(x) } +
\intf{ x_i }{ x_{i+\onehalf} }{ r(x) }
...
%и количество выделяемого тепла за счет 
%распределенных источников во всем объеме стрежня: 
% <g>
%\int\limits_{x_{i-\onehalf}}^{x_{i+\onehalf}}g(x)dx
