...
известны значения целевой функции $y_{i}=f(x_{i})$.
Произвольно выберем множество известных 
линейно независимых аналитических функций $\phi_i(x)$, 
вычисление которых не встречает серьезных трудностей.
Задача линейной интерполяции состоит в замене 
целевой функции $f(x)$ приближенной формулой, 
которая близка в некотором смысле к исходной функции:
\begin{gather}
f(x)\approx c_0\cdot\phi_0(x)+c_1\cdot\phi_1(x)+\ldots+c_m\cdot\phi_n(x)
