...
определитель системы отличен от нуля $\det\mathbf{A}\ne0$,
...
с полиномом $p_n(x)$ в точках $x_i$ и $x_{i+1}$
не гарантирует малость величины
$\norma{f(x)-p_n(x)}$ на отрезке $[x_i,x_{i+1}]$.

%
% Алгоритм интерполяции функции обобщенным линейным полиномом
%
\begin{figure}[H]\centering
\begin{scheme}
% входные данные
\node[IOnode,on chain,join]{$\{x_i\}, \{y_i\}, \{\phi_i(x)\}$};
% начало цикла по строкам
\node[rectnode,on chain,join](loop rows){$i=0$};
% начало цикла по столбцам
\node[rectnode,on chain,join](loop cols){$j=0$};
% элемент матрицы
\node[rectnode,on chain,join](matrix){$a_{ij}=\phi_j(x_i)$};
% j < n
\node[ifthenelsenode,on chain,join](compare cols){$j < n$};
\draw(compare cols.west) node[above left]{да};
\draw(compare cols.south) node[below right]{нет};
% j = j + 1
\begin{scope}[start branch=a]
\node[rectnode,on chain=going left,join](j){$j=j+1$};
\end{scope}
% i < n
\node[ifthenelsenode,on chain,join](compare rows){$i < n$};
\draw(compare rows.east) node[above right]{да};
\draw(compare rows.south) node[below right]{нет};
% i = i + 1
\begin{scope}[start branch=b]
\node[rectnode,on chain=going right,join](i){$i=i+1$};
\end{scope}
% решение системы линейных уравнений
\node[subnode,on chain,join](solve)
{\nodepart{two}$\mathbf{A}\cdot\vect{c}=\vect{y}$};
% выходные данные = решение
\begin{scope}[node distance=12mm]
\node[IOnode,on chain,join]{$\{c_0,c_1,\ldots,c_n\}$};
\end{scope}
% связи
\draw[->,thick] (j.north) |- (matrix.west);
\draw[->,thick] (i.north) |- (loop cols.east);
\end{scheme}
% *******************************************
\caption{Блок-схема алгоритма нахождения коэффициентов
$\{c_i\}$\linebreak обобщенного полинома 
$p_n(x)=\sum\limits_{i=0}^n c_i\cdot\phi_i(x)$}
\label{fig:NLE:scheme f(x)->min}
\end{figure}
...
и абсолютная погрешность интерполяции $\epsilon_i=\epsilon(x_i)$:
\begin{equation*} 
\epsilon_i=y_i-p_3(x_i).
\end{equation*} 
%
% Таблица результатов
%
...
и абсолютная погрешность интерполяции $\epsilon_i$}
...
$\epsilon_i$&$0.023$&$0.008$&$-0.022$&$-0.013$\\
\bottomrule
\end{tabular*}
\end{table}
...
node[pos=0.7,above right] {$p_3(x)$};
\end{axis}
\end{tikzpicture}
...
% Интерполяция функции заданной таблично
% алгебраическими полиномами в форме Лагранжа
\subsection{Интерполяция функции заданной таблично
алгебраическими полиномами в форме Лагранжа}
...
в этих узлах $\{y_i\}$ (таблица \ref{tab:Interpolation_Data}):
\begin{gather*}
\begin{split}
L_{3}(x)=
...
$\epsilon_i=\epsilon(x_i)$: 
\begin{equation*}
\epsilon_i=y_i-L_3(x_i).
\end{equation*}
%
% Таблица
%
...
интерполяции $\epsilon_i$}
\label{tab:Interpolation_LPD}
\small
\begin{tabular*}{\textwidth}{%
@{\extracolsep{\fill}}*{6}{r}}
\toprule
$i$&$x_i$&$y_i$&$c_i$&$L_3(x_i)$&$\epsilon_i$\\
\midmidrule
