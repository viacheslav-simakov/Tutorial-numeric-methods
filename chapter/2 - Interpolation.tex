...
\begin{scheme}
% входные данные
\node[IOnode,on chain,join]{$a, b, \epsilon$};
% x1, x2
...
\node[rectnode,on chain=going left,join](a){$a=c$};
\end{scope}
% нет
\begin{scope}[start branch=b]
\node[rectnode,on chain=going right,join](b){$b=c$};
\end{scope}
% ошибка
...
% выход?
\node[ifthenelsenode,on chain,join](exit){$\Delta<\epsilon$};
...
\node[IOnode,on chain,join]{$x_0=\dfrac{a+b}{2}$};
\end{scope}
%\node[beginendnode,on chain,join]{конец};
%\end{tikzpicture}
\end{scheme}
% *******************************************
\caption{Блок-схема алгоритма поиска решения $x_0$\linebreak
